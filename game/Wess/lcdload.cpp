...
#if defined(__XBOX__)
    extern "C" int usbh_pooling_hubs(void);  // Keep USB hub polled during long LCD loads
#endif
...
int32_t wess_dig_lcd_load(
    ...
    while ((lcdBytesLeft > 0) && (!gbWess_lcd_load_abort)) {
        ...
        #if defined(__XBOX__)
            // Poll USB hub every sector so the controller stays connected during long loads
            // (e.g. GEC ME Beta 4's large LCD files can take 10-30 seconds to transfer).
            usbh_pooling_hubs();
        #endif
