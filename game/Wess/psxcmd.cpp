...
#if defined(__XBOX__)
    // What the game's own data says the hardware has, against what it actually has.
    //
    // These have to agree. The driver hands sounds to voice indices up to this limit, and LIBSPU quietly ignores any
    // index past the number of voices the SPU was created with - so every sound landing above that is allocated, never
    // played, and reclaimed on the next tick because the SPU reports those voices as permanently off.
    //
    // This is read from the WAD, which is why it can differ between editions of the same build, and why one can lose
    // its sound effects while another keeps them.
    XBOX_LOGI(
        Audio,
        "wess hw_voice_limit=%u spu_voices=%u%s",
        (unsigned) gWess_drv_hwVoiceLimit,
        (unsigned) PsxVm::gSpu.numVoices,
        (gWess_drv_hwVoiceLimit > PsxVm::gSpu.numVoices) ? " MISMATCH - sounds above the SPU count are dropped" : ""
    );
#endif
...
#if defined(__XBOX__)
//------------------------------------------------------------------------------------------------------------------------------------------
// PsyDoom Xbox: report the state of the hardware voices, for diagnosing a leak.
//
// Voices climb to 24 over a couple of minutes and never come back down. There are exactly three places that can go
// wrong and they are distinguishable only from inside here:
//
//   1. Notes are never keyed off        -> voices stay active and never enter release
//   2. Release never completes          -> voices sit in release forever, because the clock they are timed against
//                                          is not advancing (see 'gWess_Millicount')
//   3. The SPU never reports them off   -> voices are released but the envelope never reaches its Off phase
//
// Counting each separately says which, and none of the three can be told apart from the outside.
//------------------------------------------------------------------------------------------------------------------------------------------
void PSX_GetVoiceDiagCounts(
    uint32_t& numActive,
    uint32_t& numReleasing,
    uint32_t& numSpuOff,
    uint32_t& numActiveButSpuOff
) noexcept {
    numActive = 0;
    numReleasing = 0;
    numSpuOff = 0;
    numActiveButSpuOff = 0;

    const voice_status* const pHwVoiceStats = gpWess_drv_psxVoiceStats;

    if (!pHwVoiceStats)
        return;

    uint8_t keyStatuses[SPU_NUM_VOICES] = {};
    LIBSPU_SpuGetAllKeysStatus(keyStatuses);

    const uint32_t maxHwVoices = gWess_drv_hwVoiceLimit;

    for (uint32_t i = 0; (i < maxHwVoices) && (i < SPU_NUM_VOICES); ++i) {
        const voice_status& voiceStat = pHwVoiceStats[i];
        ...
        if (voiceStat.active) {
            ...
            // This one is the smoking gun if it is ever non-zero for long: the SPU says the voice is finished and the
            // driver has not noticed, which means the reclaim loop is not running.
            if (keyStatuses[i] == SPU_OFF) {
                numActiveButSpuOff++;
            }
        }
