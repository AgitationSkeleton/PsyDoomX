...
#if defined(__XBOX__)
    // On Xbox (nxdk, single-core), use try_lock instead of blocking lock.
    // The audio callback already holds gSpuMutex. If the main thread holds gCdPlayerMutex
    // (e.g., inside psxcd_play/stop) and then tries to acquire gSpuMutex (via LIBSPU/WESS
    // timer), both threads block forever (AB-BA deadlock). Return silence instead — one missing
    // sample is inaudible, but a permanent deadlock kills the audio DPC permanently.
    if (!gCdPlayerMutex.try_lock())
        return Spu::StereoSample{};
    // RAII unlock for all return paths while holding the try-locked mutex
    struct CdUnlock { ~CdUnlock() noexcept { gCdPlayerMutex.unlock(); } } _cdUnlock;
#else
