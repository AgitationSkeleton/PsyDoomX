...
typedef std::chrono::high_resolution_clock::time_point SysTime;
    ...
    //--------------------------------------------------------------------------------------------------------------
    // A clock that works on this hardware.
    //
    // This timer is the heartbeat of the music sequencer, and measured on the console its millicount advanced to 4758
    // and then stopped for good. Voices that had begun releasing were then timed against a clock that never moved
    // again, so they could never finish releasing and were never handed back - the voice count climbed to 24, the
    // audio callback ended up mixing all of them forever, and sound effects stopped because nothing was free.
    //
    // 'std::chrono::high_resolution_clock' is the suspect: the relay's own timestamps come from
    // SDL_GetPerformanceCounter and advance perfectly across the same session, so a working clock plainly exists here.
    // Whether std::chrono is broken or the timer was switched off is settled by the diagnostic below rather than
    // assumed - but using the clock that demonstrably works is right either way.
    //--------------------------------------------------------------------------------------------------------------
    static double xboxNowSecs() noexcept {
        static uint64_t sFreq = 0;
        ...
        return (double)(SDL_GetPerformanceCounter() - sStart) / (double) sFreq;
    }
#endif
...
#if defined(__XBOX__)
    // See 'xboxNowSecs': std::chrono's clock does not keep running on this hardware, and the sequencer stops with it.
    static double sXbLastInterruptSecs = 0.0;
    const double xbNow = xboxNowSecs();
    const double elapsedSecs = (xbNow - sXbLastInterruptSecs) + gRootCnt2.spilloverTime;
    const SysTime now = std::chrono::high_resolution_clock::now();

    // Report both clocks once, a few seconds in, so 'a stopped clock' and 'a stopped timer' can be told apart rather
    // than guessed between. One line, once, costs nothing.
    {
        static bool sbReported = false;
        static SysTime sChronoStart = now;

        if ((!sbReported) && (xbNow > 8.0)) {
            sbReported = true;
            const double chronoSecs = std::chrono::duration<double>(now - sChronoStart).count();

            // Note: no %f anywhere. nxdk's printf has no floating point support, so a %f prints nothing AND consumes
            // no argument - every conversion after it then reads the wrong thing off the stack. The first version of
            // this line came back as 'sdl=s chrono=s counting=718010162 opened=1075842602', which is not a diagnostic,
            // it is noise wearing the shape of one. Everything is scaled to integers instead.
            XBOX_LOGI(
                Audio,
                "clockcheck sdl=%dms chrono=%dms counting=%d opened=%d enabled=%d interval=%dus",
                (int)(xbNow * 1000.0),
                (int)(chronoSecs * 1000.0),
                (int) gRootCnt2.bIsCounting,
                (int) gRootCnt2.bIsEventOpened,
                (int) gRootCnt2.bIsEventEnabled,
                (int)(gRootCnt2.interruptIntervalSecs * 1000000.0)
            );
        }
    }
#else
