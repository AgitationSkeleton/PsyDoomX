...
int32_t LIBSPU_SpuClearReverbWorkArea() noexcept {
    ...
    // Zero the reverb area
        ...
        // Zero the whole reverb allocation, as upstream does.
        //
        // This was narrowed to part of it to save a memset said to hold the SPU lock long enough to starve the audio
        // callback. The starvation had nothing to do with this - it was the audio ring emptying, which is dealt with in
        // 'XboxAudioOut' - and the saving is not worth what it risks: when the reverb base sits at or past the end of
        // SPU RAM the narrowed version zeroes nothing at all, leaving the reverb working area holding whatever it held
        // before. A reverb line that is never cleared feeds itself, which is heard as an echo that will not die away.
