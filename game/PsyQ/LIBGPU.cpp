...
void LIBGPU_LoadImage(const SRECT& dstRect, const uint16_t* const pImageData) noexcept {
    ...
    for (uint32_t dstY = dstTy; dstY <= dstBy; ++dstY) {
        ...
        uint16_t* const pDstRow = pVram + (intptr_t) dstYWrapped * gpu.ramStride;
...
int32_t LIBGPU_MoveImage(const SRECT& srcRect, const int32_t dstX, const int32_t dstY) noexcept {
    ...
    const uint16_t* pSrcRow = gpu.pRam + srcRect.x + (intptr_t) srcRect.y * gpu.ramStride;
    uint16_t* pDstRow = gpu.pRam + dstX + (intptr_t) dstY * gpu.ramStride;

    for (uint32_t rowIdx = 0; rowIdx < numRows; ++rowIdx) {
        std::memcpy(pDstRow, pSrcRow, rowSize);
        pSrcRow += gpu.ramStride;
        pDstRow += gpu.ramStride;
    }
