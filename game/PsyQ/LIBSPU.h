...
#if defined(__XBOX__)
    // Xbox: match the SPU that is actually created, which is the original PlayStation's 24 voices in 512 KiB.
    //
    // This build defines PSYDOOM_LIMIT_REMOVING, so this used to read 64 while 'PsxVm::init' went on creating a 24
    // voice SPU, and the two never agreed. The number here is not advisory: 'wess_load_module' overwrites the voice
    // limit the WMD file asks for with it, so the sequencer allocated voices 0 to 63 against hardware that has 24.
    //
    // Everything assigned above 23 was written into nothing. That is the silence - the relay reported "voices=0
    // active=0" for a whole session while the game was plainly trying to play sounds - and it is also the sound that
    // would not stop: a voice keyed on above the limit is keyed off above the limit too, so whatever was really
    // sounding was never released and a looping sample simply carried on.
    //
    // Limit removing stays on for everything else; only the voice count is brought back in line with the hardware.
