...
static int parseArg_server([[maybe_unused]] const int argc, const char* const* const argv) {
    if (std::strcmp(argv[0], "-server") == 0) {
        ...
        if ((argc >= 2) && (argv[1][0] != '-')) {
            ...
            if (!bValidPort) {
                printf("Bad server port number '%s'! Arg will be ignored...\n", argv[1]);
            }
...
static void validateAndSanitizeArgs() noexcept {
    if (gbHeadlessMode && (!gPlayDemoFilePath[0])) {
        printf("The '-headless' switch can only be used in conjunction with '-playdemo'! Arg will be ignored...\n");
        gbHeadlessMode = false;
    }

    if (gbRecordDemos && gPlayDemoFilePath[0]) {
        printf("Can't use '-record' in conjunction with '-playdemo'! Arg will be ignored...\n");
        gbRecordDemos = false;
    }

    if (gbIsNetClient && gbIsNetServer) {
        printf("Can't use '-server' in conjunction with '-client'! Arg will be ignored...\n");
        gbIsNetServer = false;
    }

    if (gbRecordDemos && gSaveDemoResultFilePath[0]) {
        printf("Can't use '-saveresult' in conjunction with '-record'! Arg will be ignored...\n");
        gSaveDemoResultFilePath = "";
    }

    if (gbRecordDemos && gCheckDemoResultFilePath[0]) {
        printf("Can't use '-checkresult' in conjunction with '-record'! Arg will be ignored...\n");
        gCheckDemoResultFilePath = "";
    }

    if ((gWarpMap > 0) && gPlayDemoFilePath[0]) {
        printf("The '-warp' argument conflicts with '-playdemo'! Arg will be ignored...\n");
        gWarpMap = 0;
    }
}
...
void init(const int argc, const char* const* const argv) noexcept {
    ...
    while (argsLeft > 0) {
        ...
        if (!bHandledArg) {
            printf("Unrecognized command line argument '%s'! Arg will be ignored...\n", pCurArgv[0]);
            argsLeft--;
            pCurArgv++;
        }
    }
