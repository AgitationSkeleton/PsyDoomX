...
//------------------------------------------------------------------------------------------------------------------------------------------
// Bindings, one set per local player.
//
// Splitscreen needs two players binding their controls independently, and saving them independently. Rather than
// thread a player index through every call site - almost all of which are single player and should stay that way -
// there is a set per player and one selector saying whose bindings are being read or written.
//
// The game already models two players everywhere else: MAXPLAYERS is 2, and gPlayers, gTickInputs and the co-op and
// deathmatch rules all exist. The link cable only ever carried player two's inputs, so this is the piece that lets
// those inputs come from a second pad instead.
//------------------------------------------------------------------------------------------------------------------------------------------
static BindingData gBindings[MAX_LOCAL_PLAYERS][(uint16_t) Binding::NUM_BINDINGS];

// Whose bindings the accessors below act on. Player 1 unless something says otherwise, so nothing single player changes.
static uint8_t gActivePlayer = 0;
...
// Note: on Xbox these are excluded - std::regex constructor calls std::locale::classic() which
// has a magic static that reads __declspec(thread) _Init_thread_epoch via nxdk's TLS scheme,
// causing an instant crash before main() runs on unmodified OG Xbox hardware.
...
static float getRawInput(const InputSrc src) noexcept {
    if (src.device == InputSrc::KEYBOARD_KEY) {
        ...
        // Player two reads their own pad. Everyone else reads the one the event path maintains.
        if (gActivePlayer == 1) {
            const float rawP2 = Input::getGamepadInputValueP2((GamepadInput) src.input);
            const float deadZone = Config::gGamepadDeadZone;
            const float mag = (rawP2 >= 0.0f) ? rawP2 : -rawP2;

            if (mag <= deadZone)
                return 0.0f;

            // Rescaled past the deadzone so the full range is still reachable, matching what the adjusted read does
            const float scaled = (mag - deadZone) / (1.0f - deadZone);
            return (rawP2 >= 0.0f) ? scaled : -scaled;
        }
        ...
        // Player two reads their own pad, like the axis case above
        return (gActivePlayer == 1) ? Input::getGamepadInputValueP2((GamepadInput) src.input)
                                    : Input::getGamepadInputValue((GamepadInput) src.input);
    }
    else if ((src.device == InputSrc::JOYSTICK_AXIS) ||
             (src.device == InputSrc::JOYSTICK_BUTTON) ||
             (src.device == InputSrc::JOYSTICK_HAT))
    {
        // These read whichever device was opened as player one's generic joystick, and there is no per player
        // equivalent of them. Handing that to player two would give them player one's stick, which is worse than
        // giving them nothing: it is what made a single press of pause open player two's menu, because the pause check
        // reaches player two first and saw player one's button.
        if (gActivePlayer == 1)
            return 0.0f;

        if (src.device == InputSrc::JOYSTICK_AXIS)
            return Input::getJoystickAxisValue(src.input);

        if (src.device == InputSrc::JOYSTICK_BUTTON)
...
static bool isInputJustPressed(const InputSrc src) noexcept {
    if (src.device == InputSrc::KEYBOARD_KEY) {
        ...
        return (gActivePlayer == 1) ? Input::isGamepadInputJustPressedP2((GamepadInput) src.input)
                                    : Input::isGamepadInputJustPressed((GamepadInput) src.input);
    }
    else if (src.device == InputSrc::JOYSTICK_AXIS) {
...
static bool isInputJustReleased(const InputSrc src) noexcept {
    if (src.device == InputSrc::KEYBOARD_KEY) {
        ...
        return (gActivePlayer == 1) ? Input::isGamepadInputJustReleasedP2((GamepadInput) src.input)
                                    : Input::isGamepadInputJustReleased((GamepadInput) src.input);
    }
    else if (src.device == InputSrc::JOYSTICK_AXIS) {
...
static float getInputForBinding(const Binding binding) noexcept {
    ...
    const BindingData& bindingData = gBindings[gActivePlayer][bindingIdx];
...
//------------------------------------------------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------------------------------------------------
// Choose whose bindings the accessors act on, and ask who that currently is
//------------------------------------------------------------------------------------------------------------------------------------------
void setActivePlayer(const uint8_t playerIdx) noexcept {
    gActivePlayer = (playerIdx < MAX_LOCAL_PLAYERS) ? playerIdx : 0;
}

uint8_t getActivePlayer() noexcept {
...
void clearAllBindings() noexcept {
    for (BindingData& bindingData : gBindings[gActivePlayer]) {
        bindingData = {};
    }
}
...
void parseBinding(const Binding binding, const char* const str) noexcept {
    ...
    // Clear the binding
    BindingData& bindingData = gBindings[gActivePlayer][bindingIdx];
    bindingData = {};
...
const BindingData& getBindingData(const Binding binding) noexcept {
    const uint16_t bindingIdx = (uint16_t) binding;
    return (bindingIdx < (uint16_t) Binding::NUM_BINDINGS) ? gBindings[gActivePlayer][bindingIdx] : INVALID_BINDING_DATA;
}
