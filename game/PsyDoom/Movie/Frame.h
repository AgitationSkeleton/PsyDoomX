...
public:
    ...
    // Reads past a frame without decoding it.
    //
    // Decoding is nearly all of what a frame costs, and a frame that is already too late to be shown does not need to
    // be turned into pixels - only got past, so the stream stays lined up with the audio. Every frame here is coded on
    // its own, so skipping one costs the next nothing.
    bool readSkip(CDXAFileStreamer& cdStreamer, const uint8_t channelNum) noexcept;
