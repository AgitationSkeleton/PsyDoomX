...
static void moviePlaybackLoop(const float secondsPerFrame) noexcept {
...
#if defined(__XBOX__)
    //--------------------------------------------------------------------------------------------------------------
    // Pace the video from a monotonic clock, not the wall clock.
    //
    // Video frames are chosen here by asking how much time has passed, while the audio plays at whatever rate the
    // hardware consumes samples - two independent time bases. That only stays in step if the clock the video uses is
    // steady, and 'system_clock' is the wall clock: it is not monotonic and is free to be adjusted or to step. The
    // sequencer's heartbeat had the same shape of fault and it froze the whole music system, so the same treatment.
    //
    // The real cure for audio and video drifting apart is to derive the frame from how much audio has actually been
    // played, rather than from any clock at all. That is a larger change and this measures the drift first, so it can
    // be judged on evidence rather than on my say-so.
    //--------------------------------------------------------------------------------------------------------------
    const uint64_t xbClockFreq = SDL_GetPerformanceFrequency();
    const uint64_t xbStartCounter = SDL_GetPerformanceCounter();
    uint64_t xbLastReportMicros = 0;
    int32_t xbFramesDecoded = 0;
    int32_t xbFramesSkipped = 0;
#endif
    ...
    while (shouldContinueMoviePlayback()) {
...
#if defined(__XBOX__)
        const double elapsedTimeSecs = (xbClockFreq > 0) ?
            ((double)(SDL_GetPerformanceCounter() - xbStartCounter) / (double) xbClockFreq) : 0.0;
#else
...
#if defined(__XBOX__)
        // Once a second: where the video thinks it is, and how far behind it actually is.
        //
        // 'behind' is the count of frames the clock says should have been shown that have not been - which is the
        // desync, in frames, as it happens. If that grows steadily the decode cannot keep up; if it jumps, the clock
        // did. All integers: nxdk's printf has no floating point and silently corrupts a line that uses %f.
        {
            const uint64_t nowMicros = (xbClockFreq > 0) ?
                (uint64_t)((double)(SDL_GetPerformanceCounter() - xbStartCounter) * 1000000.0 / (double) xbClockFreq) : 0;

            if (nowMicros - xbLastReportMicros >= 1000000) {
                xbLastReportMicros = nowMicros;
                XBOX_LOGI(
                    Video,
                    "fmv t=%dms tgtframe=%d shown=%d behind=%d decoded=%d skipped=%d",
                    (int)(nowMicros / 1000),
                    (int) tgtFrameIndex,
                    (int) curFrameIndex,
                    (int)(tgtFrameIndex - curFrameIndex),
                    (int) xbFramesDecoded,
                    (int) xbFramesSkipped
                );
            }
        ...
        const bool bNewFrame = (curFrameIndex < tgtFrameIndex);
        if (bNewFrame) {
            // On Xbox: refresh input before the potentially slow frame decode
            // so skip requests are detected with minimal latency.
            #if defined(__XBOX__)
                Input::update();
                if (!shouldContinueMoviePlayback())
                    break;
            #endif

            // Get past any frames that are already too late to show, then decode the one that is due.
            //
            // Measured on hardware, the decoder manages about eleven frames a second where thirty are wanted, so the
            // video fell a frame further behind roughly every tenth of a second - a hundred and twenty frames adrift
            // within six seconds, while the audio played on at its own rate. Decoding every one of those in turn could
            // never catch up, because the decoding is the thing that is too slow.
            //
            // So the late ones are read past rather than decoded. Bounded per iteration so a slow patch cannot turn
            // into a long stall, and one frame is always decoded, so this degrades into showing fewer frames rather
            // than showing none.
            #if defined(__XBOX__)
            {
                constexpr int32_t MAX_SKIP_PER_ITERATION = 8;
                int32_t framesToSkip = tgtFrameIndex - curFrameIndex - 1;

                if (framesToSkip > MAX_SKIP_PER_ITERATION) {
                    framesToSkip = MAX_SKIP_PER_ITERATION;
                }

                while (framesToSkip > 0) {
                    if (!gFrame.readSkip(gVideoFileStream, 1))
                        break;
        ...
        // On Xbox: only call SDL_RenderPresent when a new frame is ready; otherwise yield 1ms to
        // give the audio callback thread scheduling time instead of spinning uselessly.
        #if defined(__XBOX__)
            if (bNewFrame) {
    ...
    while (gbCanPlayAudioSamples && shouldContinueMoviePlayback()) {
        #if defined(__XBOX__)
            std::this_thread::sleep_for(std::chrono::milliseconds(16));  // ~60Hz cap; no need to hammer RenderPresent while draining audio
        #endif
        displayCurrentVideoFrame();
        Input::update();
    }
