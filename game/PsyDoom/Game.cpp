...
static std::chrono::high_resolution_clock::time_point gLevelStartTime;
    ...
    //
    // 'high_resolution_clock' does not run dependably on this hardware, and this is the third fault traced to it - it
    // froze the music sequencer, it turned the view on its own, and here it wrote a save file that the game then
    // refused to load. A save records how long the level has been played, that figure is checked with
    // 'secondsPlayed >= 0' when loading, and a clock that steps backwards makes it negative. The file examined held
    // -4, so the whole save failed validation and came back as "Bad save data".
    //
    // 'SDL_GetPerformanceCounter' was measured working here and is what the frame pacing, the movie player and the
    // turning were all moved onto.
    static uint64_t gXbLevelStartCounter = 0;

    // Microseconds elapsed since the level started, never negative.
    static int64_t xbLevelElapsedMicros() noexcept {
        const uint64_t nowCounter = SDL_GetPerformanceCounter();
        const uint64_t freq = SDL_GetPerformanceFrequency();

        if ((freq == 0) || (nowCounter <= gXbLevelStartCounter))
            return 0;

        return (int64_t)(((nowCounter - gXbLevelStartCounter) * 1000000ull) / freq);
    }
#endif
...
void startLevelTimer() noexcept {
    ...
    #if defined(__XBOX__)
        gXbLevelStartCounter = SDL_GetPerformanceCounter();
    #endif
...
void setLevelElapsedTimeMicrosecs(const int64_t elapsedUsec) noexcept {
    ...
    #if defined(__XBOX__)
        // Wind the counter back by the same amount, so a loaded save carries on from where it left off
        const uint64_t freq = SDL_GetPerformanceFrequency();
        const uint64_t counterNow = SDL_GetPerformanceCounter();
        const uint64_t elapsedCounts = (freq > 0) ? (uint64_t)((elapsedUsec > 0) ? elapsedUsec : 0) * freq / 1000000ull : 0;
        gXbLevelStartCounter = (counterNow > elapsedCounts) ? (counterNow - elapsedCounts) : 0;
    #endif
}
