...
// Player two's pad, for splitscreen: whether one is connected, and what it currently reads.
//
// Polled rather than listened to, because the event path keeps one flat array of values with no notion of which
// controller an event came from. See the note by the definition.
bool hasSecondGamepad() noexcept;

// Report the input devices the console presents, to the relay
