...
void init() noexcept {
    ...
    try {
        ...
        printf("PsyDoom: error executing the map script! Details follow:\n%s\n", e.what());
    ...
    if (actionIter != gScriptActions.end()) {
        try {
            ...
            if (!result.valid()) {
                ...
                printf("PsyDoom: error executing map script action #%d! Details follow:\n%s\n", actionNum, error.what());
            }
        }
        catch (const std::exception& e) {
            showStatusBarError("Script #%d error! See stdout.", actionNum);
        ...
        printf("PsyDoom: no scripting action #%d is available to execute!\n", actionNum);
    }
