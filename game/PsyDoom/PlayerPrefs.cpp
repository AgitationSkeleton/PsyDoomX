...
void save() noexcept {
    ...
    if (!pFile) {
        // Xbox: say so. A file that would not open and a file that was never written to look identical afterwards.
        #if defined(__XBOX__)
...
#if defined(__XBOX__)
    // Both players' settings are written, unlike turn speed and autorun which only keep player one's.
    //
    // Those predate splitscreen and were left alone; there is no reason for a new setting to inherit that, and a second
    // player who has to pick their colour again on every boot would notice.
    std::fprintf(pFile, "ssgStyle = %d\n", (int) gSsgStyle[0]);
    std::fprintf(pFile, "ssgStyle2 = %d\n", (int) gSsgStyle[1]);
    std::fprintf(pFile, "playerColour = %d\n", (int) gPlayerColour[0]);
    std::fprintf(pFile, "playerColour2 = %d\n", (int) gPlayerColour[1]);
#endif
    ...
    {
        char msg[128];
        std::snprintf(msg, sizeof(msg), "prefs: written - ssg %d/%d colour %d/%d",
            (int) gSsgStyle[0], (int) gSsgStyle[1], (int) gPlayerColour[0], (int) gPlayerColour[1]);
        xbLog(msg);
    }
    #endif
