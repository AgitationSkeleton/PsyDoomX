...
void initCfgSerialization_Graphics() noexcept {
    ...
    cfg.enhanceWallDrawPrecision = makeConfigField(
        ...
        "\n"
        "Off by default on Xbox: it switches the wall renderer's arithmetic to 64-bit, which that console's\n"
        "32-bit CPU has to synthesise from several instructions per operation, in the innermost drawing loop.\n"
        "What it buys is an improvement on the original PlayStation's own precision, so going without it is a\n"
        "step towards how the game originally looked rather than away from it.",
        gbEnhanceWallDrawPrecision,
        // GAMECUBE: the reasoning holds there too - it is also a 32-bit CPU without native 64-bit integer
        // arithmetic - but the figure wants measuring on that hardware rather than inheriting this one.
        #if defined(__XBOX__)
            false
        #else
            true
