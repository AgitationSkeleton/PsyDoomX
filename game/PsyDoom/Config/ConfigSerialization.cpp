...
// Player two's bindings, kept in a file of their own.
//
// A second file rather than a second section, so that player two's controls survive independently: one player
// rebinding cannot disturb the other's, and a file that goes missing or is hand-edited badly only costs that player
// their bindings. The fields are the same set, read and written with player two selected as the active player.
static constexpr const char* const CFG_FILE_CONTROLS_P2 = "control_bindings_p2.ini";
...
void readAllConfigFiles() noexcept {
    ...
    // Controls always come from the built-in defaults on this port.
    //
    // There is no rebinding screen, so the stored file can only ever hold some previous build's defaults - and reading
    // it silently overrode any change made to them. That is why the new pad layout appeared not to take: the file on
    // the console still held the old one, and it won every time. The file is still written, so it shows what is
    // actually in use, but it is no longer read back.
    #if defined(__XBOX__)
        {
            const ConfigFieldList defaultControlFields = gConfig_Controls.getFieldList();

            for (size_t i = 0; i < defaultControlFields.numFields; ++i) {
                defaultControlFields.pFieldList[i].defInitFunc();
            }
        }

        const bool bNeedSave_Controls = true;
    #else
        const bool bNeedSave_Controls       = readConfigFile(cfgDir, CFG_FILE_CONTROLS,     gConfig_Controls.getFieldList());
    #endif

    // Player two's bindings go into their own set. Selecting the player first is what decides which set the binding
    // fields write into, then it goes back so everything after this is player one as before.
    bool bNeedSave_ControlsP2 = false;
    {
        Controls::setActivePlayer(1);

        // Give player two the default bindings before reading their file.
        //
        // Player two had no working controls at all, and this is why: nothing ever established their bindings. The
        // config read only applies a default where a field is missing from a file that exists, so a player whose file
        // was written once - by this very code, from an empty set - keeps that empty set for good. Every binding then
        // has no input sources, which returns zero for ever and is indistinguishable from a pad that is not there.
        //
        // Defaulting the whole set first means the file can only ever override it, never leave it blank.
        const ConfigFieldList controlFields = gConfig_Controls.getFieldList();

        for (size_t i = 0; i < controlFields.numFields; ++i) {
            controlFields.pFieldList[i].defInitFunc();
        }

        // Player two's defaults have just been applied above; their file is not read back for the same reason
    bNeedSave_ControlsP2 = true;
    (void) CFG_FILE_CONTROLS_P2;
    ...
    Config::gbNeedSave_Controls     |= (bNeedSave_Controls || bNeedSave_ControlsP2);
...
void writeAllConfigFiles(const bool bWriteUnchangedConfig) noexcept {
    ...
    // ...and player two's, from their own set of bindings
    {
        Controls::setActivePlayer(1);
        bool bNeedSaveP2 = Config::gbNeedSave_ControlsP2;
        maybeWriteConfig(CFG_FILE_CONTROLS_P2, gConfig_Controls.getFieldList(), bNeedSaveP2, CONTROL_BINDINGS_INI_HEADER);
        Config::gbNeedSave_ControlsP2 = bNeedSaveP2;
        Controls::setActivePlayer(0);
    }
