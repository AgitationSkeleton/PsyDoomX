...
void W_Init() noexcept {
    ...
    // Xbox: the other editions' super shotgun sprites, if the launcher managed to take them off their discs.
    //
    // These carry sprite names of their own rather than overriding anything, so being in the list costs nothing until
    // the setting asks for them. See 'SsgStyle.h'. Not portable: there is no launcher on other platforms to write them.
    #if defined(__XBOX__)
