...
// PsyDoom limit removing: we now dynamically make this bigger if needed, based on the size of the .WMD file.
    ...
    #if defined(__XBOX__)
        static uint8_t*                  gSound_WmdMem = nullptr;
    #else
...
void S_LoadMapSoundAndMusic(const int32_t mapNum) noexcept {
    ...
    // Xbox: begin long-load guard.  GEC ME Beta 4 has very large LCD files; the uploads
    // can block for 10-30 seconds, starving the audio callback (SDL_AUDIO_STOPPED) and
    // the USB hub poll (controller drops).  This pauses audio and keeps USB alive.
    #if defined(__XBOX__)
        PsxVm::beginLongLoad();
    #endif
    ...
    // Xbox: end long-load guard — stops keepalive thread, resumes audio, ensures device playing.
    #if defined(__XBOX__)
        PsxVm::endLongLoad();
    #endif
}
...
static void I_QueueSound(mobj_t* const pOrigin, const sfxenum_t soundId) noexcept {
    ...
    #if defined(__XBOX__)
        // Splitscreen: listen from whichever player is closer to the sound.
        //
        // There is one set of speakers and two players, and everything below attenuates against a single listener. With
        // that listener fixed to player one, player two's own gunshots were scored on how far away player one was, so
        // they were quiet or clipped entirely - which is why player two appeared to have no sound of their own while
        // still being able to fire. Whoever is nearer owns the sound; a sound made by one of them is heard at full
        // volume by whoever made it, and pans correctly for that player.
        if (Splitscreen::isActive() && pOrigin) {
            mobj_t* const pOther = gPlayers[(gCurPlayerIndex == 0) ? 1 : 0].mo;

            if (pListener && pOther) {
                const auto approxDistTo = [pOrigin](const mobj_t& listener) noexcept -> fixed_t {
                    const fixed_t dx = std::abs(listener.x - pOrigin->x);
                    const fixed_t dy = std::abs(listener.y - pOrigin->y);
                    return dx + dy - d_rshift<1>(std::min(dx, dy));
                };

                // The sound's own maker always wins, otherwise the nearer of the two
                if (pOrigin == pOther) {
                    pListener = pOther;
                } else if ((pOrigin != pListener) && (approxDistTo(*pOther) < approxDistTo(*pListener))) {
                    pListener = pOther;
                }
            }
            else if (!pListener) {
    ...
    if (bAttenuateSound) {
        ...
        #if defined(__XBOX__)
            // Splitscreen: lean each player's sounds towards the half of the screen they are watching.
            //
            // There is one pair of speakers for two players, so a sound cannot be private to one of them. What it can
            // do is come from their side: with the viewports side by side, player one is on the left and player two on
            // the right, so their sounds are placed there. That is enough to tell whose gunshot or whose pickup it was
            // without having to look.
            //
            // Only for the side by side layout. One above the other puts both players in the same horizontal place,
            // where a left or right bias would say nothing about who a sound belonged to and would just be wrong.
            if (Splitscreen::isActive() && (Splitscreen::getLayout() == Splitscreen::Layout::SideBySide)) {
                const int32_t listenerIdx = (pListener == gPlayers[1].mo) ? 1 : 0;

                // Halve the in-world panning to leave room for the side bias, then push to that player's side
                const int32_t sideBias = (listenerIdx == 1) ? (WESS_PAN_CENTER / 2) : -(WESS_PAN_CENTER / 2);
                pan = WESS_PAN_CENTER + ((pan - WESS_PAN_CENTER) / 2) + sideBias;
                pan = std::clamp<int32_t>(pan, WESS_PAN_LEFT, WESS_PAN_RIGHT);
            }
        #endif
...
void PsxSoundInit(const int32_t sfxVol, const int32_t musVol, void* const pTmpWmdLoadBuffer) noexcept {
    ...
    // This should allow memory to grow if custom mods have very complex music tracks that consume lots of memory.
        ...
        #if defined(__XBOX__)
            free(gSound_WmdMem);
            gSound_WmdMem = (uint8_t*)malloc(gSound_WmdMemSize);
            wess_load_module(pTmpWmdLoadBuffer, gSound_WmdMem, gSound_WmdMemSize, gSound_SettingsLists);
        #else
