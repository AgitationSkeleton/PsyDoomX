...
#include <algorithm>
    ...
    void xbLog(const char* msg) noexcept {
        // Truncate the log file on the very first write of each run so entries don't accumulate across runs
        ...
        if (h != INVALID_HANDLE_VALUE) {
            ...
            //
            // 'FILE_APPEND_DATA' alone is not honoured here: every write was landing at offset zero and overwriting the
            // one before it, so the file only ever held fragments of whichever line happened to be longest. A boot log
            // of 69 bytes after a full run is what gave it away. That affected every DOOM_STEP trace as well, so boot
            // logs from this port have been unreliable the whole time rather than just for the GPU work.
...
#if PSYDOOM_MODS
#if defined(__XBOX__)
static uint32_t gXboxAppStartTick = 0;
#endif
...
#if defined(__XBOX__)
    // Set while splitscreen draws a screen once per player; the present happens once, after both
    bool gbXbSuppressPresent = false;
#endif
...
void I_Main() noexcept {
        ...
        #if defined(__XBOX__)
            // On nxdk, std::chrono::steady_clock::now() returns a stalled value, breaking
            // I_GetTotalVBlanks() and thus the entire game tick rate. Use SDL_GetTicks()
            // (millisecond resolution, proven to work) as the time base instead.
            gXboxAppStartTick = SDL_GetTicks();
        #endif
    #endif

    DOOM_STEP("I_Main: LIBSN__main");
    LIBSN__main();
    DOOM_STEP("I_Main: D_DoomMain");
    D_DoomMain();
    DOOM_STEP("I_Main: returned");
}
...
void I_PSXInit() noexcept {
    ...
    DOOM_STEP("I_PSXInit: LIBETC_ResetCallback");
    LIBETC_ResetCallback();

    // Initialize the GPU
    DOOM_STEP("I_PSXInit: LIBGPU_ResetGraph");
    LIBGPU_ResetGraph(0);
    LIBGPU_SetGraphDebug(0);

    // Initialize the gamepad
    DOOM_STEP("I_PSXInit: LIBAPI_InitPAD");
    ...
    DOOM_STEP("I_PSXInit: LIBGTE_InitGeom");
    LIBGTE_InitGeom();
...
void I_DrawLoadingPlaque(texture_t& tex, const int16_t xpos, const int16_t ypos, const int16_t clutId) noexcept {
    // PsyDoom: ignore in headless mdoe
    ...
    #if defined(__XBOX__)
        // Splitscreen: the plaque belongs on both players' halves.
        //
        // Drawn once, it appeared for whichever half the draw area happened to point at and the other player got
        // nothing. The front to back copy below is skipped too: with splitscreen on, those two buffers are the two
        // viewports rather than a front and a back, so copying one over the other would put one player's picture on
        // top of the other's.
        if (Splitscreen::isActive()) {
            ...
            gbXbSuppressPresent = true;

            for (int32_t i = 0; i < MAXPLAYERS; ++i) {
                if (!gbPlayerInGame[i])
                    continue;

                Splitscreen::pointDrawAt(i);        // On top of that player's view, not clearing it
                I_DrawSprite(tex.texPageId, clutId, xpos, ypos, tex.texPageCoordX, tex.texPageCoordY, tex.width, tex.height);
                I_SubmitGpuCmds();
            }

            gbXbSuppressPresent = false;

            // One present for the pair
            Splitscreen::pointDrawAt(0);
            I_DrawPresent();
            return;
        }
    #endif
...
void I_DrawPresent() noexcept {
    #if defined(__XBOX__)
        // Splitscreen is drawing the same screen twice, once per player, and presents once itself afterwards.
        //
        // Menu drawers each present at their own tail, which is right when there is one of them and wrong when two
        // run per frame. Suppressing it here covers every drawer without having to touch each one.
        if (gbXbSuppressPresent)
            return;
    #endif
    ...
    //
    // Xbox splitscreen does not swap. Both framebuffers hold a live view - player one in the first, player two in the
    // second - so there is no spare buffer to alternate into, and alternating moves the draw area and the displayed
    // area under two views that are both still needed. Every frame then differs from the last in where things land,
    // which is the constant flicker.
    //
    // Nothing is lost by holding still: the present writes to the Xbox framebuffer directly and reads these only as
    // source rectangles, so there is no tearing to double buffer against in the first place.
    #if defined(__XBOX__)
        const bool bSwapFramebuffers = (!Splitscreen::isActive());
    #else
        const bool bSwapFramebuffers = true;
    #endif

    if (bSwapFramebuffers) {
        gCurDispBufferIdx ^= 1;
    ...
    // PsyDoom: copy the PSX framebuffer to the display
        ...
        #if defined(__XBOX__)
        // Everything from here to the end of this function is timed as 'pace'.
        //
        // 'other' is 8.4ms of a 31.9ms frame with nothing accounting for it, and this is what is inside it. Worth
        // separating because one part of it is a deliberate sleep: 'threadYield' is 'SDL_Delay(1)' on this platform,
        // taken every frame, and the reason given for it was to let SDL's audio thread be scheduled - a thread this
        // port no longer uses, having replaced it with one of its own that asks for the highest priority there is.
        // If that sleep is still costing a millisecond or more a frame it is being paid for nothing.
        const uint64_t xbPaceStart = XboxLog::nowMicros();

        // On Xbox, unconditionally yield + process platform updates right after display.
        // At slow frame rates (>33ms) the vblank spin below exits immediately and SDL_Delay
        // is never called, starving the SDL audio thread and USB hub polling. This ensures
        // Input::update() (and thus usbh_pooling_hubs) and the audio thread always get CPU.
        Utils::doPlatformUpdates();
        Utils::threadYield();
        #endif
    #endif
...
bool I_NetUpdate() noexcept {
    // Splitscreen: there is no network, and both players' inputs are already in hand.
    //
    // Everything below exists to keep two consoles in step: an error check word to catch them diverging, and inputs
    // sent a tick ahead so the wire has time to deliver them. Neither means anything when there is one simulation and
    // both pads are plugged into it. Player two's inputs were gathered alongside player one's in 'D_DoomMain', so
    // running any of this would only overwrite them.
    //
    // Returning false says the game has not been aborted. Skipping the tick-ahead scheme also removes the frame of
    // input lag it costs, which a link cable game has no choice but to accept.
    #if defined(__XBOX__)
        if (Splitscreen::isActive())
            return false;
    #endif
    ...
    // you are now allowed to turn at any time outside of the regular 30 Hz update loop for the player.
    {
        ...
        ASSERT((gPlayerUncommittedTurning[gCurPlayerIndex] == 0) || (gbGamePaused));
        const angle_t playerAngle = (bInGame) ? gPlayers[gCurPlayerIndex].mo->angle : 0;
        gPlayerNextTickViewAngle[gCurPlayerIndex] = playerAngle + gTickInputs[gCurPlayerIndex].getAnalogTurn() + gNextTickInputs.getAnalogTurn();
    }
...
#if defined(__XBOX__)
    // nxdk's std::chrono::steady_clock does not advance reliably. Use SDL_GetTicks()
    // (milliseconds since SDL init) and convert to 50Hz or 60Hz ticks.
    const uint32_t elapsedMs = SDL_GetTicks() - gXboxAppStartTick;
    const uint32_t netAdjustMs = (gNetGame != gt_single) ? (uint32_t) gNetTimeAdjustMs : 0u;
    const uint32_t totalMs = elapsedMs + netAdjustMs;
    if (Game::gSettings.bUsePalTimings) {
        return (int32_t)((uint64_t) totalMs * 50u / 1000u);
    } else {
        return (int32_t)((uint64_t) totalMs * 60u / 1000u);
    }
#else
