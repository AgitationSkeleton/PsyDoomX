...
void Z_Init() noexcept {
        ...
        int32_t heapSize = (Config::gMainMemoryHeapSize <= 0) ? Z_HEAP_DEFAULT_SIZE : Config::gMainMemoryHeapSize;
    #else
        int32_t heapSize = Z_HEAP_SIZE;
    #endif

    // Xbox: cap heap at 24 MB to leave room for kernel, GPU, stack and game code in 64 MB total RAM
    #if defined(__XBOX__)
        constexpr int32_t XBOX_MAX_HEAP = 24 * 1024 * 1024;
        if (heapSize > XBOX_MAX_HEAP) { heapSize = XBOX_MAX_HEAP; }
        free(gZoneHeap);
        gZoneHeap = malloc((size_t)heapSize);
        gpMainMemZone = Z_InitZone(gZoneHeap, heapSize);
    #else
