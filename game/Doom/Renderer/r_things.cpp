...
void R_DrawSubsectorSprites(subsector_t& subsec) noexcept {
    ...
    for (const vissprite_t* pSpr = gVisSpriteHead.next; pSpr != &gVisSpriteHead; pSpr = pSpr->next) {
        ...
        // Xbox: draw a player in whichever colour they chose, the way PC Doom does.
        //
        // A recoloured copy of the marine's frames, standing in for the ones this object would otherwise use. Decided
        // from 'mobj_t::player', which only a player's own object has - so the dead marines and gibs that came with the
        // level are untouched, and stay the green they were drawn in. See 'PlayerColour.h'.
        #if defined(__XBOX__)
            const spritedef_t* const pColouredDef = PlayerColour::spriteDefForThing(thing);
            const spritedef_t& spriteDef = (pColouredDef) ? *pColouredDef : gSprites[thing.sprite];
        #else
