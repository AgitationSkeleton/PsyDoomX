...
// PsyDoom: used for interpolation when the framerate is uncapped
    ...
    #if defined(__XBOX__)
        // The same two moments, on a clock this hardware actually keeps.
        //
        // The interpolation mix is 'now minus when this tick began', and those two came from different places: 'now'
        // from 'std::chrono::steady_clock', the tick start from a synthetic timeline built out of the vblank count.
        // Neither is wrong alone. Together they are a difference between a clock nxdk does not advance dependably -
        // the same one that froze the music sequencer and the platform update throttle, both since moved off it - and
        // an ideal timeline that never varies at all. What comes out is a mix factor that jumps about.
        //
        // That is seen as every interpolated object jittering while the player watching them does not, because the
        // viewing player's own camera is snapped rather than mixed. In a one player game there is rarely anything
        // moving smoothly enough to notice it on; in splitscreen the other player is on screen the whole time.
        //
        // 'SDL_GetTicks' is the clock the rest of this port settled on for exactly this reason. Its millisecond step
        // is far finer than the world frame it divides into - 67 milliseconds - and finer than the jitter it replaces.
        static uint32_t gXbPlayerFrameStartMs = 0;
        static uint32_t gXbWorldFrameStartMs = 0;
    #endif
    ...
    fixed_t     gOldViewXs[MAXPLAYERS];
    fixed_t     gOldViewYs[MAXPLAYERS];
    fixed_t     gOldViewZs[MAXPLAYERS];
    angle_t     gOldViewAngles[MAXPLAYERS];
    fixed_t     gOldAutomapX;
    fixed_t     gOldAutomapY;
    fixed_t     gOldAutomapScale;
    bool        gbSnapViewZInterpolations[MAXPLAYERS];
    ...
    fixed_t gViewPushedZs[MAXPLAYERS];
    ...
    bool gbOldViewZIsPusheds[MAXPLAYERS];
#endif
...
void R_RenderPlayerView() noexcept {
    ...
    if (bInterpolateFrame) {
        ...
        if (bUserControlsTurning) {
            ...
            // Splitscreen uses the local angle, like single player does.
            //
            // 'gPlayerNextTickViewAngle' is a network idea: the angle this console said it would be at next tick, sent
            // ahead so the other machine is not a frame behind. It is only ever updated inside 'I_NetUpdate', which
            // splitscreen skips because there is no network - so using it here left the view angle frozen. That is
            // what stopped player one turning, and it also made movement look wrong: the player object's angle went on
            // changing while the view did not, so walking forward went somewhere other than the way the screen faced.
            #if defined(__XBOX__)
                const bool bUseLocalViewAngle = ((gNetGame == gt_single) || Splitscreen::isActive());
            #else
                const bool bUseLocalViewAngle = (gNetGame == gt_single);
            #endif

            if (bUseLocalViewAngle) {
                gViewAngle = playerMobj.angle + gPlayerUncommittedTurning[gCurPlayerIndex];
            } else {
                gViewAngle = gPlayerNextTickViewAngle[gCurPlayerIndex] + gPlayerUncommittedTurning[gCurPlayerIndex];
            }
        } else {
...
void R_InterpBeginPlayerFrame() noexcept {
    ...
    #if defined(__XBOX__)
        gXbPlayerFrameStartMs = SDL_GetTicks();
    #endif
...
void R_InterpBeginWorldFrame() noexcept {
    ...
    #if defined(__XBOX__)
        gXbWorldFrameStartMs = SDL_GetTicks();
    #endif
}
...
void R_CalcLerpFactors() noexcept {
    ...
    #if defined(__XBOX__)
        // Both ends of the subtraction from one clock - see the note where these are declared
        const uint32_t nowMs = SDL_GetTicks();
        const double netTimeAdjustSecs = ((gNetGame != gt_single) ? gNetTimeAdjustMs : 0) / 1000.0;
        const double playerElapsedSeconds = ((double)(uint32_t)(nowMs - gXbPlayerFrameStartMs) / 1000.0) + netTimeAdjustSecs;
        const double worldElapsedSeconds = ((double)(uint32_t)(nowMs - gXbWorldFrameStartMs) / 1000.0) + netTimeAdjustSecs;
    #else
