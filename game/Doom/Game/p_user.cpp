...
static bool gbOnGround;
    ...
    angle_t gPlayerUncommittedTurning[MAXPLAYERS];
    ...
    angle_t gPlayerNextTickViewAngle[MAXPLAYERS];

    // When we last did framerate uncapped turning movements for the current player
    static time_point_t gLastPlayerTurnTime[MAXPLAYERS];

    #if defined(__XBOX__)
        // Xbox: the same instant, taken from a clock that can be trusted.
        //
        // 'high_resolution_clock' does not run dependably on this hardware. It is what froze the music sequencer, where
        // one step backwards left a comparison that could never come true again, and turning reads it the same way, so
        // a bad reading turns the view instead of stopping the music. 'SDL_GetPerformanceCounter' was measured working
        // here and is what the frame pacing and the movie player were both moved onto.
        static uint64_t gLastPlayerTurnCounter[MAXPLAYERS] = {};

        // Largest step turning will act on at once, in 60 Hz ticks.
        //
        // The shared code clamps to 60, a whole second of turning in a single update. That is fine where frames are
        // short and the clock is sane, but here the gap across a map load is seconds long, so the first update after
        // one loads spends the full second of turn in one go, in whatever direction the stick happens to be resting -
        // which is the hard snap on spawning in, and the turn that takes a moment to settle.
        //
        // Six ticks is 100ms. Frames here run about 36ms, so ordinary turning never comes near this and is unchanged;
        // it only bites on the long gaps, and there is nothing worth catching up on across those anyway - the stick
        // says where it is pointing now, not where it was pointing during the load.
        static constexpr float XBOX_MAX_TURN_TICKS = 6.0f;

        // Xbox: how far the turn stick has to be pushed before it counts as pushed at all.
        //
        // The view drifts round on its own after spawning in. Two readings of the same fault, from two sessions:
        //
        //     turn 3 right=105 left=0 axis=105 ticks60=3032 turnAmt=50   <- a tenth deflected, untouched
        //     turn 7 right=44  left=0 axis=44  ticks60=2635 turnAmt=17   <- and later, a twentieth
        //
        // The stick reads deflected while nobody is touching it. That is the pad's centre being off, not anything the
        // turning code does with the value, which is why clamping the time step did nothing.
        //
        // Waiting for the stick to be seen at rest before trusting it did not work either, and the second reading above
        // is from that build: it does pass through centre, so the wait ends, and then it goes back to reading a
        // twentieth deflected and the drift resumes. The rest position is not steady, so no one-off check can stand in
        // for a threshold.
        //
        // A deadzone is what this needs. It is applied on top of the one 'Controls::getFloat' already does, because
        // that one is sized for a pad in good order and these readings are past it. Anything under this counts as
        // centred; anything over is rescaled so that the full range is still reachable and the response stays smooth
        // rather than jumping as it crosses.
        //
        // NOT PORTABLE TO GAMECUBE as a value: the principle carries over, since those sticks wander too, but the
        // amount is this pad's and the constant wants measuring again there rather than copying.
        static constexpr float XBOX_TURN_DEADZONE = 0.15f;

        // Xbox: trace turning for the first few seconds after spawning in.
        //
        // The view drifts right on loading into a map and settles after a moment, and clamping the time step did not
        // stop it, so the step is not what is driving it. Rather than guess a fourth time, this reports what turning is
        // actually being handed each update - the stick reading, the step, and what came out - so the cause can be read
        // off instead of inferred. About three seconds at the frame rate this runs at.
        static uint32_t gXbTurnTraceCount = 0;
        static constexpr uint32_t XBOX_TURN_TRACE_UPDATES = 90;
    #endif
#endif
    ...
    if (mobj.z <= mobj.floorz) {
        ...
        if (mobj.momz < 0) {
            if (mobj.momz < -GRAVITY * 2) {
                // If we hit the floor hard then play the 'oof' sound and do the hard floor impact (if allowed)
...
void P_PlayerThink(player_t& player) noexcept {
    ...
    // Grab the current and previous inputs
        ...
        #if defined(__XBOX__)
            // What player two's own think function actually sees.
            //
            // Every other link is now verified: their pad reaches gTickInputs[1] (fwd=65019 was measured), their logic
            // runs every tick at the same rate as player one's, they are alive, they have a map object and they have
            // psprites. The only thing never measured is this array at the moment this function reads it - between the
            // gather and here sits a whole frame, and something in it could be clearing them.
            if ((gPlayerNum == 1) && Splitscreen::isActive()) {
                static uint32_t sLastSeen = 0;

                if (gTicCon - sLastSeen >= 60) {
                    sLastSeen = gTicCon;

                    XBOX_LOGI(
                        Split,
                        "p2 think sees: fwd=%d side=%d turn=%d attack=%d use=%d | reactiontime=%d momxy=%d,%d",
                        (int) inputs.getAnalogForwardMove(),
                        (int) inputs.getAnalogSideMove(),
                        (int) inputs.getAnalogTurn(),
                        (int) bAttack,
                        (int) bUse,
                        (int)(player.mo ? player.mo->reactiontime : -1),
                        (int)(player.mo ? player.mo->momx >> 16 : 0),
                        (int)(player.mo ? player.mo->momy >> 16 : 0)
                    );
                }
            }
        #endif
...
void P_PlayerInitTurning() noexcept {
    ...
    for (int32_t i = 0; i < MAXPLAYERS; ++i) {
        gLastPlayerTurnTime[i] = std::chrono::high_resolution_clock::now();

        #if defined(__XBOX__)
            gLastPlayerTurnCounter[i] = SDL_GetPerformanceCounter();
        #endif

        gPlayerUncommittedTurning[i] = 0;
        gPlayerNextTickViewAngle[i] = (gPlayers[i].mo) ? gPlayers[i].mo->angle : 0;
    }
}
...
void P_PlayerDoTurning(const int32_t playerIdx) noexcept {
    ...
    if (bCanTurn) {
        ...
        #if defined(__XBOX__)
            // Measured from the performance counter, and clamped hard. See the notes on 'XBOX_MAX_TURN_TICKS'.
            const uint64_t nowCounter = SDL_GetPerformanceCounter();
            const uint64_t counterFreq = SDL_GetPerformanceFrequency();

            // Guard the reversed case as well as the oversized one. A counter that has gone backwards would otherwise
            // wrap to an enormous unsigned value and turn the view hard.
            const float timeDeltaF = (
                ((counterFreq > 0) && (nowCounter > gLastPlayerTurnCounter[playerIdx])) ?
                    (float)(double)(nowCounter - gLastPlayerTurnCounter[playerIdx]) / (float)(double) counterFreq :
                    0.0f
            );

            const float ticks60F = std::min(timeDeltaF * 60.0f, XBOX_MAX_TURN_TICKS);
        #else
            const float timeDeltaF = std::chrono::duration<float>(now - gLastPlayerTurnTime[playerIdx]).count();
        ...
        if (!inputs.fStrafe()) {
            ...
            turnAmt = FixedMul((fixed_t)((float) turnAmt * PlayerPrefs::getTurnSpeedMultiplier(playerIdx)), ticks60);

            if (!bFinalDoomMovementMode) {
                turnAmt /= VBLANKS_PER_TIC;
            }

            gPlayerUncommittedTurning[playerIdx] += (angle_t) d_lshift<TURN_TO_ANGLE_SHIFT>(turnAmt);
        }
        ...
        // Do analog controller turning from the gamepad
        {
            ...
            #if defined(__XBOX__)
                // Ignore a stick that is only slightly off centre - it is resting, not being pushed.
                // See the note on 'XBOX_TURN_DEADZONE'.
                {
                    const float axisMag = std::abs(axis);

                    if (axisMag <= XBOX_TURN_DEADZONE) {
                        axis = 0.0f;
                    } else {
                        // Rescaled so the range past the deadzone still reaches full deflection, and so the response
                        // grows from nothing rather than stepping straight to a sixth of the turn speed
                        const float scaled = (axisMag - XBOX_TURN_DEADZONE) / (1.0f - XBOX_TURN_DEADZONE);
                        axis = (axis > 0.0f) ? scaled : -scaled;
                    }
                }
            #endif
            ...
            const float turnSpeed = (turnSpeedLow * (1.0f - turnSpeedMix) + turnSpeedHigh * turnSpeedMix) * PlayerPrefs::getTurnSpeedMultiplier(playerIdx);
            ...
            #if defined(__XBOX__)
                // Trace the first few seconds after spawning in. Everything is scaled to integers on purpose: nxdk's
                // printf has no floating point support at all, and a '%f' in it prints nothing while also consuming no
                // argument, which silently corrupts every conversion after it.
                if (gXbTurnTraceCount < XBOX_TURN_TRACE_UPDATES) {
                    gXbTurnTraceCount++;

                    const float axisRight = Controls::getFloat(Controls::Binding::Analog_TurnRight);
                    const float axisLeft = Controls::getFloat(Controls::Binding::Analog_TurnLeft);

                    XBOX_LOGT(
                        Input,
                        "turn %u right=%d left=%d axis=%d ticks60=%d turnAmt=%d uncommitted=%u angle=%u",
                        gXbTurnTraceCount,
                        (int)(axisRight * 1000.0f),
                        (int)(axisLeft * 1000.0f),
                        (int)(axis * 1000.0f),
                        (int)(ticks60F * 1000.0f),
                        (int) turnAmt,
                        (unsigned) gPlayerUncommittedTurning[playerIdx],
                        (unsigned) player.mo->angle
                    );
                }
            #endif
        }
        ...
        // Do turning from the mouse and consume the movements after we have counted them
        {
            ...
            const float turnSpeed = Config::gMouseTurnSpeed * PlayerPrefs::getTurnSpeedMultiplier(playerIdx);
    ...
    // Note this happens whether or not turning was allowed, so that time spent paused or dead is not banked up and
    // spent as a lurch on the first update after play resumes.
    gLastPlayerTurnTime[playerIdx] = now;

    #if defined(__XBOX__)
        gLastPlayerTurnCounter[playerIdx] = SDL_GetPerformanceCounter();
    #endif
}
...
void P_UncommitTurningTickInputs() noexcept {
    ...
    if (gNetGame == gt_single) {
        gPlayerUncommittedTurning[gCurPlayerIndex] += tickInputs.getAnalogTurn();
        tickInputs.setAnalogTurn(0);
    } else {
        TickInputs& nextTickInputs = gNextTickInputs;
        gPlayerUncommittedTurning[gCurPlayerIndex] += tickInputs.getAnalogTurn();
        gPlayerUncommittedTurning[gCurPlayerIndex] += nextTickInputs.getAnalogTurn();
        gPlayerNextTickViewAngle[gCurPlayerIndex] -= tickInputs.getAnalogTurn();         // Don't double count, remove the turning from this since it's already in there
        gPlayerNextTickViewAngle[gCurPlayerIndex] -= nextTickInputs.getAnalogTurn();
