...
#if defined(__XBOX__)
    extern uint32_t gXbP2PauseEdges;
    extern bool gbXbP2PauseLatched;
    extern bool gbXbP2MenuBackLatched;
#endif
...
#if defined(__XBOX__)
    // How many times each player's logic has actually run.
    //
    // Player two's inputs are proven correct all the way into gTickInputs[1] - the relay shows fwd=65019 and fire=1 -
    // and P_PlayerThink reads gTickInputs[gPlayerNum], so a player who still does not move, consume ammo, make a sound
    // or hold a weapon is a player whose logic is not running. This counts it instead of arguing about it.
    uint32_t gXbPlayerThinkCount[MAXPLAYERS] = {};

// How many times each player's viewport is actually drawn.
//
// Both views are drawn unconditionally in 'P_Drawer', so if these do not match each other, or do not match the number
// of frames presented, something is reaching the screen without a fresh pair behind it - which is what flicker is.
uint32_t gXbViewDraws[2] = {};
#endif
...
void P_CheckCheats() noexcept {
    ...
    for (int32_t playerIdx = MAXPLAYERS - 1; playerIdx >= 0; --playerIdx) {
        ...
        // Toggling pause?
        #if PSYDOOM_MODS
            bool bPauseJustPressed = (inputs.fTogglePause() && (!oldInputs.fTogglePause()));

            #if defined(__XBOX__)
                // Take player two's held press, however many frames ago it happened
                if ((playerIdx == 1) && Splitscreen::isActive() && gbXbP2PauseLatched) {
                    gbXbP2PauseLatched = false;
                    bPauseJustPressed = true;
                }
            #endif
        #else
        ...
        #if defined(__XBOX__)
            // Whether each player's pause edge ever reaches the place that acts on it.
            //
            // Player two's start button is confirmed reaching their pad array with a clean 0 to 1 transition, their
            // Toggle_Pause binding has sources, and the edge test compares the two arrays correctly - yet nothing
            // happens. This loop is the only thing left between that edge and the game pausing, and it runs per tick
            // while the gather runs per frame, so an edge landing on a non tick frame would be dropped unseen.
            if (Splitscreen::isActive() && (inputs.fTogglePause() || oldInputs.fTogglePause())) {
                XBOX_LOGI(
                    Split,
                    "pause edge: player %d now=%d was=%d -> acting=%d (paused=%d)",
                    (int) playerIdx, (int) inputs.fTogglePause(), (int) oldInputs.fTogglePause(),
                    (int) bPauseJustPressed, (int) gbGamePaused
                );
            }
        #endif
        ...
        #if defined(__XBOX__)
            // Take player two's held select press, however many frames ago it happened
            if ((playerIdx == 1) && Splitscreen::isActive() && gbXbP2MenuBackLatched) {
                gbXbP2MenuBackLatched = false;
                bMenuBackJustPressed = true;
            }
        #endif
        ...
        #if defined(__XBOX__)
            // Say who opened this, and whose bindings it will edit.
            //
            // Pausing as player one has been opening what looks like player two's menu, and the two candidate causes
            // look identical from outside: the wrong player triggering it, or the right player triggering it and the
            // menu then reading the wrong player's settings. The loop above runs player two first, so either is
            // possible. This distinguishes them.
            XBOX_LOGI(
                Split,
                "options menu opened by player %d (view player %d, active bindings player %u)",
                (int) playerIdx,
                (int) gCurPlayerIndex,
                (unsigned) Controls::getActivePlayer()
            );
        #endif

        // Run the options menu.
        //
        // Xbox: the menu belongs to whoever opened it.
        //
        // Two things decide that and both were wrong. 'Controls' reads whichever player is active, which was always
        // player one, so player two's pad could not drive the menu and player two rebinding would have edited player
        // one's controls. And the menu indexes its own cursor by 'gCurPlayerIndex', so both players shared one cursor.
        //
        // Setting both to the player who pressed makes it theirs by construction: their pad moves the cursor, their
        // bindings are what a controls screen edits, and their cursor position is remembered separately. Restored
        // afterwards so the game goes back to drawing and reading as player one.
        #if defined(__XBOX__)
            const int32_t savedViewPlayer = gCurPlayerIndex;
            const uint8_t savedActivePlayer = Controls::getActivePlayer();

            if (Splitscreen::isActive()) {
                gCurPlayerIndex = playerIdx;
                Controls::setActivePlayer((uint8_t) playerIdx);

                // Draw this player's menu into this player's half of the screen (single player menu path only).
                //
                // The menu runs its own loop and draws over whichever half the draw area happens to point at. That
                // used to alternate every frame with the framebuffer swap, which put the menu in both halves on
                // alternate frames - it looked like both players had a menu, and stopped looking that way once the
                // swap was pinned to stop the flicker. Pointing it at the owner's half makes it actually theirs.
                //
                // Without clearing: the other half keeps the last frame of that player's view, which the present
                // blits every frame regardless, so the game stays on screen for whoever is not in the menu.
                Splitscreen::pointDrawAt(playerIdx);
            }
        #endif

        // Splitscreen opens BOTH menus, whichever player pressed select.
        //
        // The menu is a blocking loop, so one loop per player would mean the second player waiting for the first to
        // finish - and only one menu ever on screen, which is what was happening. One loop driving both lets each
        // player work their own menu, with their own cursor and their own bindings, at the same time.
        #if defined(__XBOX__)
            const gameaction_t optionsAction = (Splitscreen::isActive()) ?
                MiniLoop(O_Init, O_Shutdown, O_ControlSplit, O_DrawerSplit) :
                MiniLoop(O_Init, O_Shutdown, O_Control, O_Drawer);
        #else
        ...
        #if defined(__XBOX__)
            if (Splitscreen::isActive()) {
                gCurPlayerIndex = savedViewPlayer;
...
#if defined(__XBOX__)
//------------------------------------------------------------------------------------------------------------------------------------------
// PsyDoom Xbox: what the unaccounted part of a frame is actually made of.
//
// A frame measures about 63ms. 26ms of it is putting the picture on the screen and is understood. The other 38ms is
// simply everything else that happens between two presents, and reaching 30fps means most of it has to go - but there
// is no point optimising a number nobody has broken down. These separate the emulated PlayStation rasteriser from the
// game's own logic, which are the two candidates and want very different treatment.
//------------------------------------------------------------------------------------------------------------------------------------------
uint64_t gXbRenderMicros = 0;       // R_RenderPlayerView: the emulated rasteriser drawing the 3D view
uint64_t gXbTickMicros = 0;         // P_Ticker: the game's own simulation
uint64_t gXbPaceMicros = 0;         // The tail of 'I_DrawPresent': platform updates, the yield, and the vblank spin
#endif

gameaction_t P_Ticker() noexcept {
#if defined(__XBOX__)
    const uint64_t xbTickStart = XboxLog::nowMicros();
    struct XbTickTimer {
        uint64_t start;
        ~XbTickTimer() noexcept { gXbTickMicros = XboxLog::nowMicros() - start; }
    } xbTickTimer { xbTickStart };
#endif
    gGameAction = ga_nothing;

    #if PSYDOOM_MODS
        // PsyDoom: do framerate uncapped turning for the current player
        P_PlayerDoTurning(gCurPlayerIndex);

        #if defined(__XBOX__)
            // Splitscreen: player two turns on this console too
            if (Splitscreen::isActive()) {
                P_PlayerDoTurning(1);
            }
        #endif
        ...
        #if defined(__XBOX__)
            // Both players, not just whichever one is current.
            //
            // These record where a view is interpolating away FROM, and there is a set of them per player now. Snapping
            // only the current one would leave the other view blending away from wherever that player was several ticks
            // ago, which is the same fault in a slower form.
            if (Splitscreen::isActive()) {
    ...
    if ((!gbGamePaused) && (gGameTic > gPrevGameTic)) {
        ...
        #if defined(__XBOX__)
            // Splitscreen: player two's status bar ticks too, so their face follows their own powers and health
            if (Splitscreen::isActive()) {
                const int32_t savedPlayer = gCurPlayerIndex;
                gCurPlayerIndex = 1;
                ST_Ticker();
    ...
    for (gPlayerNum = 0; gPlayerNum < MAXPLAYERS; gPlayerNum += 1) {
        ...
        #if defined(__XBOX__)
            gXbPlayerThinkCount[gPlayerNum]++;
        #endif
...
//------------------------------------------------------------------------------------------------------------------------------------------
#if defined(__XBOX__)
// Times 'R_RenderPlayerView' wherever it is called from, so both the stereo and the ordinary path are covered
// Accumulates, so both views count.
//
// This assigned instead, so with two views a frame the second one overwrote the first and player one's rendering
// vanished from the accounting entirely - it was landing in 'other', which is why 'other' stayed large and why the
// reported render time looked like it alternated between two values. 'gXbRenderMicros' is zeroed once per frame by
// the present, after it has been read.
struct XbRenderTimer {
    uint64_t start;
    XbRenderTimer() noexcept : start(XboxLog::nowMicros()) {}
    ~XbRenderTimer() noexcept { gXbRenderMicros += XboxLog::nowMicros() - start; }
};
#endif
...
void P_Drawer() noexcept {
    ...
    #if defined(__XBOX__)
        // Player one draws into their own half of VRAM, which splitscreen clears for them.
        //
        // Needed for two reasons. The draw area otherwise alternates between the two framebuffers every frame for
        // double buffering, which would put player one in player two's half every other frame. And the automatic
        // clear is off while splitscreen is on, because it was wiping already drawn views, so the clear has to happen
        // here instead.
        if (Splitscreen::isActive()) {
            Splitscreen::beginPlayerView(0);
            gXbViewDraws[0]++;

            // Pick the screen tint for the player about to be drawn.
            //
            // 'I_UpdatePalette' reads 'gCurPlayerIndex' and is normally called from 'ST_Drawer', which for each view
            // runs after that view has been rendered - so every view was drawn with the other player's tint left over
            // from the previous frame. Damage flashes, item pickup flashes and the berserk tint all landed on the
            // wrong half of the screen. Choosing it here means each view is drawn with its own player's tint.
            I_UpdatePalette();

            // And the super shotgun this player carries.
            //
            // Same reason as the tint: there is one sprite list between the two views, so whichever set was pointed at
            // last would otherwise be the one both players saw. Ten frame records copied per view.
            SsgStyle::applyForPlayer(0);
        }
    #endif
    ...
    #if defined(__XBOX__)
        XboxLog::gXbStatusBarMicros = 0;    // Reset for the frame; both draws below accumulate into it
        {
            const unsigned long long sbStart = XboxLog::nowMicros();
            ST_Drawer();
            XboxLog::gXbStatusBarMicros += XboxLog::nowMicros() - sbStart;
        }
    #else
        ST_Drawer();
    #endif
    ...
    #if defined(__XBOX__)
        // Splitscreen: draw the world again through player two's eyes, into their own half of VRAM.
        //
        // Two regions, drawn once each, read once each, nothing copied and nothing shared. The game already has two
        // 256x240 framebuffers at (0,0) and (256,0) which it alternates for double buffering; splitscreen uses both at
        // once, which costs nothing because the present writes to the Xbox framebuffer directly and only reads these
        // as source rectangles.
        if (Splitscreen::isActive()) {
            I_SubmitGpuCmds();      // Finish player one's view before the draw area moves

            gCurPlayerIndex = 1;
            Splitscreen::beginPlayerView(1);
            gXbViewDraws[1]++;
            I_UpdatePalette();              // Player two's own tint, now that they are the current player
            SsgStyle::applyForPlayer(1);    // And their own super shotgun

            // Start a new frame as far as the renderer's per-frame caches are concerned.
            //
            // This is what put player one's world inside player two's view. 'R_BSP' caches each vertex's transformed
            // screen position on the vertex itself and keys that cache on 'gNumFramesDrawn':
            //
            //      if (segv1.frameUpdated != gNumFramesDrawn) { ...transform... segv1.frameUpdated = gNumFramesDrawn; }
            //
            // 'gNumFramesDrawn' only advances in 'I_DrawPresent', which runs after both views. So every vertex player
            // one's view already transformed was still marked current when player two's view reached it, and got reused
            // carrying player one's camera transform. Only geometry both views can see is shared, which is why this
            // appeared exactly when the two players were near each other in the same sectors, and why the views looked
            // like two worlds intersecting rather than simply wrong.
            //
            // Advancing it here invalidates those caches so player two's view transforms everything through its own
            // camera. It is also correct for the texture cache, the other user of this counter: it treats textures from
            // the current frame as un-evictable, and player one's view has already been submitted by this point, so its
            // textures are legitimately last frame's.
            gNumFramesDrawn++;

            // Note: player two's view no longer needs anchoring here.
            //
            // It used to, because the values a view interpolates away from were one set shared between both players, so
            // player two's view was blended back towards player one's camera and at a small mix factor landed on top of
            // it. This code answered that by pinning player two's 'previous' view to their CURRENT one, which stopped
            // the two views merging but also meant player two's view never interpolated at all.
            //
            // There is a set of those values per player now, snapped for both at the tick, so the view is interpolated
            // properly rather than pinned. See 'gOldViewXs' in 'r_main.h'.
            {
                XbRenderTimer xbRenderTimer;
                R_RenderPlayerView();
            }
            ...
            {
                const unsigned long long sbStart2 = XboxLog::nowMicros();
                ST_Drawer();
                XboxLog::gXbStatusBarMicros += XboxLog::nowMicros() - sbStart2;
            }

            I_SubmitGpuCmds();      // Finish player two's view before anything else draws

            gCurPlayerIndex = 0;

            // Report what each player actually is, once a second.
            //
            // Inputs are proven correct into gTickInputs[1], and P_PlayerThink reads gTickInputs[gPlayerNum], so a
            // player two who still does not move must be one whose logic never runs - or one whose weapon was never
            // set up, which is the same signature as no ammo, no sound and no weapon sprite. Both are stated here
            // rather than argued about.
            {
                static uint32_t sLastReportTic = 0;

                if (gTicCon - sLastReportTic >= 60) {
                    sLastReportTic = gTicCon;

                    const player_t& p1 = gPlayers[0];
                    const player_t& p2 = gPlayers[1];

                    XBOX_LOGI(
                        Split,
                        "views=%u,%u frames=%u | players: ingame=%d,%d thinkruns=%u,%u state=%d,%d hasmo=%d,%d psprite=%d,%d pos=%d,%d/%d,%d",
                        gXbViewDraws[0], gXbViewDraws[1], gNumFramesDrawn,
                        (int) gbPlayerInGame[0],
                        (int) gbPlayerInGame[1],
                        gXbPlayerThinkCount[0],
                        gXbPlayerThinkCount[1],
                        (int) p1.playerstate,
                        (int) p2.playerstate,
                        (int)(p1.mo != nullptr),
                        (int)(p2.mo != nullptr),
                        (int)(p1.psprites[0].state != nullptr),
                        (int)(p2.psprites[0].state != nullptr),
                        (int)(p1.mo ? p1.mo->x >> 16 : 0),
                        (int)(p1.mo ? p1.mo->y >> 16 : 0),
                        (int)(p2.mo ? p2.mo->x >> 16 : 0),
                        (int)(p2.mo ? p2.mo->y >> 16 : 0)
                    );
                }
            }

            // Put drawing back in player one's half without clearing it.
            //
            // Everything after this - plaques, menus, the pause screen - draws wherever the draw area was left, and
            // leaving it in player two's half is why pausing as player one put the menu on player two's screen. The
            // clear has to be avoided here specifically: player one's view is already drawn and waiting to be shown,
            // and clearing on the way back is what turned their viewport black.
            {
                DRAWENV& env = gDrawEnvs[0];
...
void P_Start() noexcept {
    ...
    M_ClearRandom();
        ...
        // And the same for player two, whose values are their own - see the note in 'P_Ticker'
        #if defined(__XBOX__)
            if (gPlayers[1].mo) {
                const int32_t savedPlayer = gCurPlayerIndex;
                gCurPlayerIndex = 1;
...
void P_GatherTickInputs(TickInputs& inputs) noexcept {
    ...
    if (Controls::isJustPressed(Controls::Binding::Toggle_Autorun)) {
        PlayerPrefs::gbAlwaysRun[Controls::getActivePlayer()] = (!PlayerPrefs::gbAlwaysRun[Controls::getActivePlayer()]);
        gStatusBar.messageTicsLeft = 15;
        gStatusBar.message = (PlayerPrefs::gbAlwaysRun[Controls::getActivePlayer()]) ? "Autorun ON." : "Autorun OFF.";
    }

    // If we're always running then this gets inverted
    if (PlayerPrefs::gbAlwaysRun[Controls::getActivePlayer()]) {
    ...
    #if defined(__XBOX__)
        // Weapon groups on the d-pad, in the style of the 3DS port.
        //
        // Each direction owns a pair and toggles within it, exactly as fist and chainsaw already do above: press once
        // for the first weapon, again for the other. Falls back to the pair's other weapon when the preferred one is
        // not owned, so a direction always gives whatever of its pair the player actually has.
        //
        // Not portable as-is: this is the pad layout for this port. The plain direct switches below are what other
        // platforms use, and the keyboard number keys still reach every weapon individually here too.
        {
            // Whoever is being gathered for, not whoever the console belongs to.
            //
            // 'player' above is gPlayers[gCurPlayerIndex], which is player one throughout the gather, so player two's
            // pairs were being decided from player one's inventory and ready weapon.
            const player_t& pairPlayer = gPlayers[Controls::getActivePlayer()];

            const auto switchWithinPair = [&](const Controls::Binding binding, const weapontype_t a, const weapontype_t b) noexcept {
                if (!Controls::getBool(binding))
                    return;

                // Toggle to the other of the pair, unless the player does not own it
                const bool bHasA = pairPlayer.weaponowned[a];
                const bool bHasB = pairPlayer.weaponowned[b];

                if (pairPlayer.readyweapon == a) {
                    inputs.directSwitchToWeapon = (uint8_t)((bHasB) ? b : a);
                } else {
                    inputs.directSwitchToWeapon = (uint8_t)((bHasA) ? a : b);
                }
            };

            switchWithinPair(Controls::Binding::Weapon_SuperShotgun,   wp_supershotgun, wp_shotgun);   // D-pad up
            switchWithinPair(Controls::Binding::Weapon_RocketLauncher, wp_missile,      wp_chainsaw);  // D-pad down
            switchWithinPair(Controls::Binding::Weapon_Chaingun,       wp_chaingun,     wp_pistol);    // D-pad right
            switchWithinPair(Controls::Binding::Weapon_PlasmaRifle,    wp_plasma,       wp_bfg);       // D-pad left
    ...
    //
    // For whoever is being gathered for, not for whoever owns the console: splitscreen gathers player two through
    // this same function with the active player switched, and their turning has to follow their own inputs.
    const int32_t turnPlayerIdx = (int32_t) Controls::getActivePlayer();

    P_PlayerDoTurning(turnPlayerIdx);
    ...
    if (!gbGamePaused) {
        inputs.setAnalogTurn(inputs.getAnalogTurn() + gPlayerUncommittedTurning[turnPlayerIdx]);
        gPlayerUncommittedTurning[turnPlayerIdx] = 0;
    }
}
