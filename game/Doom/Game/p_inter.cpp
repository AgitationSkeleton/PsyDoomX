...
void P_TouchSpecialThing(mobj_t& special, mobj_t& toucher) noexcept {
    ...
    const auto playItemPickupSound = [&]() noexcept {
        ...
        #if defined(__XBOX__)
            // Either player sitting here should hear what they picked up
            const bool bIsLocal = Splitscreen::isLocalPlayer((int32_t)(&player - gPlayers));
        #else
            const bool bIsLocal = (&player == &gPlayers[gCurPlayerIndex]);
        #endif
...
void P_KillMobj(mobj_t* const pKiller, mobj_t& target) noexcept {
    ...
    if (pTargetPlayer) {
        ...
        if (target.health < -50) {
            ...
            // Whoever was gibbed, not whoever the console belongs to.
            //
            // 'gStatusBar' resolves through gCurPlayerIndex, which is player one for the whole tick, so this both
            // tested the wrong player and wrote the wrong status bar - player two being gibbed set nothing at all.
            {
                const int32_t gibbedIdx = (int32_t)(pTargetPlayer - gPlayers);
