...
void G_DoReborn(const int32_t playerIdx) noexcept {
    ...
    if (gPlayers[playerIdx].mo->player) {
        // Xbox: keep the player's colour on the body before the link back to them is broken.
        //
        // From here on there is nothing left on the body to say whose it was, so the renderer could not work out what
        // colour to draw it. PC Doom does not have this problem because its marker is a flag on the object rather than
        // a pointer, and it keeps the body coloured - which is the behaviour being matched. See 'PlayerColour.h'.
        #if defined(__XBOX__)
            PlayerColour::markCorpse(*gPlayers[playerIdx].mo, playerIdx);
        #endif

        gPlayers[playerIdx].mo->player = nullptr;
    }

    // Xbox: give this player their face back.
    //
    // The gibbing animation ends by switching the face off, and only the once-per-level startup ever switched it back
    // on - so a player who was gibbed had an empty box on their status bar for the rest of the map.
    #if defined(__XBOX__)
        ST_RestartPlayerFace(playerIdx);
    #endif
...
void G_InitNew(const skill_t skill, const int32_t mapNum, const gametype_t gameType) noexcept {
    ...
    #if PSYDOOM_MODS
        // Splitscreen determines its settings here too.
        //
        // The comment above is true of a link cable game: the two consoles agree settings during the connect, so by
        // this point they are already decided. Splitscreen skips that connect - there is nobody to agree with - so
        // without this the settings would be left as whatever the last game set them to.
        #if defined(__XBOX__)
            const bool bDetermineSettings = ((gNetGame == gt_single) || Splitscreen::isActive());
        #else
            const bool bDetermineSettings = (gNetGame == gt_single);
        #endif

        if (bDetermineSettings && (!gpDemo_p)) {
