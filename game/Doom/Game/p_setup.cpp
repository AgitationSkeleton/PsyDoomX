...
//------------------------------------------------------------------------------------------------------------------------------------------
// PsyDoom: activates all boss related specials in the map
//------------------------------------------------------------------------------------------------------------------------------------------
static void P_ActivateAllBossSpecials() noexcept {
...
//------------------------------------------------------------------------------------------------------------------------------------------
#if defined(__XBOX__)
    // How many things the map declared. Kept so the level report can say how much of the map's content is being
    // rasterised, which is the thing that decides how heavy a map is to draw.
    static int32_t gXbNumMapThings = 0;
#endif
...
static void P_LoadThings(const int32_t lumpNum) noexcept {
    ...
    #if defined(__XBOX__)
        gXbNumMapThings = numThings;    // For the level report sent to the relay
    #endif
    W_ReadMapLump(lumpNum, pTmpBufferBytes, true);
...
void P_SetupLevel(const int32_t mapNum, [[maybe_unused]] const skill_t skill) noexcept {
    ...
    #if defined(__XBOX__)
        // Say what has just been loaded, and how much of it there is.
        //
        // Frame times swing widely between maps - 'render' has been seen anywhere from 13 to 25ms - and until now the
        // log gave no way to tell which map a session was in, let alone how heavy it was. Line and sector counts say
        // how much geometry the renderer has to walk, and the thing count how much is being drawn on top of it, which
        // together is most of what decides whether a map holds its frame rate.
        {
            const MapInfo::Map* const pMapInfo = MapInfo::getMap(mapNum);
            const char* const mapName = (pMapInfo) ? pMapInfo->name.c_str().data() : "?";
            const char* gameName = "Doom";

            switch (Game::gGameType) {
                case GameType::Doom:                        gameName = "Doom";                  break;
                case GameType::FinalDoom:                   gameName = "Final Doom";            break;
                case GameType::GEC_ME_Beta3:                gameName = "Master Edition B3";     break;
                case GameType::GEC_ME_Beta4:                gameName = "Master Edition B4";     break;
                case GameType::GEC_ME_TestMap_Doom:         gameName = "ME Test (Doom)";        break;
                case GameType::GEC_ME_TestMap_FinalDoom:    gameName = "ME Test (Final Doom)";  break;
            }

            XBOX_LOGI(
                General,
                "level %s MAP%02d %s - %d linedefs, %d sectors, %d things (%d of which are monsters)",
                gameName,
                (int) mapNum,
                mapName,
                (int) gNumLines,
