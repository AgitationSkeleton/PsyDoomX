...
#if defined(__XBOX__)
        mobjInfo.radius = (fixed_t)(XboxStrConv::parseDouble(tokenStr.c_str() + 7, nullptr) * 65536.0);
#else
        mobjInfo.radius = (fixed_t)(std::strtod(tokenStr.c_str() + 7, nullptr) * 65536.0);
#endif
    }
    else if (tokenStr.find("HEIGHT:") == 0) {
#if defined(__XBOX__)
        mobjInfo.height = (fixed_t)(XboxStrConv::parseDouble(tokenStr.c_str() + 7, nullptr) * 65536.0);
#else
        mobjInfo.height = (fixed_t)(std::strtod(tokenStr.c_str() + 7, nullptr) * 65536.0);
#endif
    }
    else {
