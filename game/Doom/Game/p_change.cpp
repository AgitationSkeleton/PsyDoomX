...
bool P_ThingHeightClip(mobj_t& mobj) noexcept {
    ...
    #if PSYDOOM_MODS
        if (mobj.player) {
            // Whichever player was moved, not just the console's own.
            //
            // With two players on one console, crediting this to 'gCurPlayerIndex' meant a lift under player two
            // pushed player one's view instead - and vice versa - so the player standing still was the one that
            // jittered.
            const int32_t movedPlayerIdx = (int32_t)(mobj.player - gPlayers);

            if ((movedPlayerIdx >= 0) && (movedPlayerIdx < MAXPLAYERS)) {
                gViewPushedZs[movedPlayerIdx] += mobj.z - oldZ;
            }
        }
    #endif
...
bool PIT_ChangeSector(mobj_t& mobj) noexcept {
    ...
    if (mobj.health <= 0) {
        ...
        // Whichever player was crushed, not just the console's own
        {
            const int32_t gibbedIdx = (int32_t)(mobj.player - gPlayers);

            if ((gibbedIdx >= 0) && (gibbedIdx < MAXPLAYERS)) {
                gStatusBars[gibbedIdx].gotgibbed = true;
            }
        }
