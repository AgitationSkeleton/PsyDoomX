...
#if defined(__XBOX__)
    // PsyDoom Xbox: how long the emulated rasteriser and the game tick took, for the frame breakdown. See 'p_tick.cpp'.
    extern uint64_t gXbRenderMicros;
    extern uint64_t gXbTickMicros;

    // ...and what 'I_DrawPresent' does after handing over the picture: platform updates, the yield, and the vblank spin.
    //
    // This is the last large piece of the frame with no name on it. 'other' measures 8.4ms of a 31.9ms frame and this
    // is inside it, including a 'SDL_Delay(1)' taken every single frame - a sleep whose stated purpose was to let SDL's
    // audio thread be scheduled, which is a thread this port no longer uses.
    extern uint64_t gXbPaceMicros;
#endif
