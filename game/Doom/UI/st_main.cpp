...
stbar_t     gStatusBars[MAXPLAYERS];    // The main state for the status bar, one per player

// Face related state
int32_t                 gFaceTicsArr[MAXPLAYERS];       // Ticks left for current face, per player
// Draw the face sprite? One per player.
//
// The gib animation clears this when it finishes, so shared it meant one player being gibbed stopped the other
// player's face from drawing as well.
bool                    gbDrawSBFaceArr[MAXPLAYERS];
const facesprite_t*     gpCurSBFaceSprites[MAXPLAYERS]; // Which sprite to draw, per player
// Are we animating the face being gibbed? One per player.
//
// The frames themselves live in the status bar and were already per player, but this flag - the one that says an
// explosion is running at all - was shared, so one player being gibbed set both faces animating.
bool                    gbGibDrawArr[MAXPLAYERS];
bool                    gbDoSpclFaceArr[MAXPLAYERS];    // Should we do a special face next?
int32_t                 gNewFaceArr[MAXPLAYERS];        // Which normal face to use next
spclface_e              gSpclFaceTypeArr[MAXPLAYERS];   // Which special face to use next

#if defined(__XBOX__)
//------------------------------------------------------------------------------------------------------------------------------------------
// Put a player's face back after they have respawned - see the note in the header
//------------------------------------------------------------------------------------------------------------------------------------------
void ST_RestartPlayerFace(const int32_t playerIdx) noexcept {
    if ((playerIdx < 0) || (playerIdx >= MAXPLAYERS))
        return;

    // These are all reached through 'gCurPlayerIndex', which is not necessarily the player being reborn
...
static void ST_DrawRightAlignedStat(const int32_t xMargin, const int32_t y, const char statChar, const int32_t statAmt, const int32_t statTotal) noexcept {
    ...
    I_DrawStringSmall(x, y, strBuffer, Game::getTexClut_STATUS(), 128, 128, 128, false, true);
}
