...
//------------------------------------------------------------------------------------------------------------------------------------------
// Splitscreen: the intermission for both players, each in their own half.
//
// Everything on this screen is already written from the current player's point of view - 'gCurPlayerIndex' picks which
// column is YOU and which is HIM, and the frag table and status lines follow the same rule. Drawing it once per player
// with that set is all it takes for each of them to see the tally from their own side.
//------------------------------------------------------------------------------------------------------------------------------------------
void IN_DrawerSplit() noexcept {
    ...
    Splitscreen::pointDrawAt(savedPlayer);
    I_DrawPresent();
}
