...
gameaction_t RunMenu() noexcept {
    do {
        ...
        // Xbox: a two player game here is splitscreen, so there is nobody to connect to.
        //
        // Both players are already sat at this console. Everything the connect below does - the plaque, the handshake,
        // the abort loop - exists to find a second machine and agree a start with it, and none of it means anything
        // when the second player is on the other end of the sofa. Falling through to it is what made choosing co-op
        // fail instantly with a network error.
        //
        // Skipping straight out starts the game exactly as single player does; 'Splitscreen::begin' below is what
        // brings the second view up.
        #if defined(__XBOX__)
            break;
        #endif
    ...
    // PsyDoom: don't do this if app quit was requested.
    {
        ...
        if (bStartGame) {
            ...
            // player one has chosen co-op or deathmatch and confirmed. It comes down again below, where a link cable
            // game would shut its network down.
            #if defined(__XBOX__)
                if (gStartGameType != gt_single) {
                    Splitscreen::begin();
                }
