...
//------------------------------------------------------------------------------------------------------------------------------------------
#if defined(__XBOX__)
    // Which screen each player is looking at. Submenus are state per player rather than nested loops, so that one
    // player opening Extra Options does not stop the other player's menu from running.
    enum class PlayerSubMenu : uint8_t {
        None,
        XOptions,
    };

    static PlayerSubMenu gPlayerSubMenu[MAXPLAYERS] = {};
#endif
...
void O_Init() noexcept {
    ...
    //
    // Splitscreen gets the full paused menu rather than the cut down network one. The network menu drops controls and
    // sensitivity because two consoles cannot sensibly rebind mid-game against each other - but both players here are
    // sat at the same machine with their own pads and their own saved bindings, so there is nothing to stop either of
    // them changing their own. Which player's bindings get edited follows from the active player.
    #if defined(__XBOX__)
        const bool bUseNetGameMenu = ((gNetGame != gt_single) && (!Splitscreen::isActive()));
    #else
        const bool bUseNetGameMenu = (gNetGame != gt_single);
    #endif
    ...
    for (int32_t playerIdx = MAXPLAYERS - 1; playerIdx >= 0; --playerIdx) {
        ...
        // PsyDoom: this is now replaced by the 'load game' or 'load and save' menu option.
            ...
            case opt_extra_options: {
                if (bMenuOk) {
                    #if defined(__XBOX__)
                        // Splitscreen: enter the submenu as state, not as a loop.
                        //
                        // A nested MiniLoop here blocks the loop driving both players, so whichever player opened
                        // this froze the other player's menu until they came back out. Recording which submenu each
                        // player is in lets the shared loop tick and draw the right screen for each of them.
                        if (Splitscreen::isActive()) {
                            // Whose submenu this is: the player whose input it was.
                            //
                            // The loop around this runs over 'playerIdx' and covers BOTH players on every call, while
                            // 'gCurPlayerIndex' is whichever player the caller happens to be on - and they are not the
                            // same. Both of these acted on the current player, so player two choosing this while player
                            // one was the current one opened player one's submenu and then drove player one's menu with
                            // player two's stick. 'XOptions_Init' reads it too, hence moving it rather than only the
                            // line below.
                            const int32_t savedPlayer = gCurPlayerIndex;
                            gCurPlayerIndex = playerIdx;

                            XOptions_Init();
                            gPlayerSubMenu[playerIdx] = PlayerSubMenu::XOptions;
...
void O_Drawer() noexcept {
    ...
    // Finish up the frame.
    //
    // Skipped while splitscreen is drawing both menus: that draws this twice, once into each half, and must present
    // once afterwards rather than once per menu.
    I_SubmitGpuCmds();
    I_DrawPresent();   // Does nothing while splitscreen is drawing both menus; it presents once for the pair
}

#if defined(__XBOX__)

//------------------------------------------------------------------------------------------------------------------------------------------
// Splitscreen: run the options menu for both players at once.
//
// The menu is a blocking loop, so running it per player would mean one player waiting for the other to finish. Both
// players get their menu at the same time instead, each driven by their own pad and each drawn into their own half.
// Every piece of menu state is already indexed per player - the cursor by 'gCurPlayerIndex', the bindings by the
// active player - so the two run side by side without interfering.
//------------------------------------------------------------------------------------------------------------------------------------------
gameaction_t O_ControlSplit() noexcept {
    gameaction_t action = ga_nothing;
    ...
    for (int32_t i = 0; i < MAXPLAYERS; ++i) {
        ...
        gCurPlayerIndex = i;
        Controls::setActivePlayer((uint8_t) i);

        if (gPlayerSubMenu[i] == PlayerSubMenu::XOptions) {
            // This player is in a submenu: tick that instead, and drop them back to the options menu when they leave
            const gameaction_t subAction = XOptions_Update();

            if (subAction != ga_nothing) {
                XOptions_Shutdown(subAction);
                gPlayerSubMenu[i] = PlayerSubMenu::None;
            }

            continue;   // A player in a submenu cannot also be closing the pause menu
        }

        const gameaction_t playerAction = O_Control();

        // Either player leaving closes the menu for both
        if (playerAction != ga_nothing) {
            action = playerAction;
        }
    }
    ...
    // Leaving the pause menu entirely takes any open submenu with it
    if (action != ga_nothing) {
        for (int32_t i = 0; i < MAXPLAYERS; ++i) {
            if (gPlayerSubMenu[i] == PlayerSubMenu::XOptions) {
                XOptions_Shutdown(action);
                gPlayerSubMenu[i] = PlayerSubMenu::None;
            }
        }
    }
...
void O_DrawerSplit() noexcept {
    ...
    for (int32_t i = 0; i < MAXPLAYERS; ++i) {
        ...
        gCurPlayerIndex = i;
        Splitscreen::pointDrawAt(i);        // Over the top of that player's view, not clearing it
