...
void F1_Start() noexcept {
    #if defined(__XBOX__)
        // One full size view: this is shown to both players, not once each
        if (Splitscreen::isActive()) {
            Splitscreen::beginFullScreenSequence();
        }
    #endif
...
void F1_Stop([[maybe_unused]] const gameaction_t exitAction) noexcept {
    #if defined(__XBOX__)
        Splitscreen::endFullScreenSequence();   // Back to two viewports when gameplay resumes
    #endif
