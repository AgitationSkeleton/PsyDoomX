...
//------------------------------------------------------------------------------------------------------------------------------------------
// Step texture coordinates in fixed point rather than floating point.
//
// On for Xbox only. The saving comes from that console's CPU having no SSE, where converting a float to an integer
// costs tens of cycles because of the rounding mode dance the C cast requires. Machines with SSE do that conversion in
// a single instruction and have nothing to gain, so they keep the original code and its exact arithmetic.
//
// Set to 0 to put the rasteriser back exactly as it was.
//
// GAMECUBE: worth turning on there too - its CPU has no such conversion either - but measure before assuming.
//------------------------------------------------------------------------------------------------------------------------------------------
#if defined(__XBOX__)
    #include <xmmintrin.h>

    // How far ahead to ask for texels. Enough rows that the memory has time to arrive, not so many that the prefetch
    // lands outside anything this column will actually read.
    static constexpr int32_t XB_TEX_PREFETCH_ROWS = 8;

    #define XB_PREFETCH(ADDR) _mm_prefetch((const char*)(ADDR), _MM_HINT_T0)
#else
    #define XB_PREFETCH(ADDR) ((void) 0)
#endif

#if !defined(PSYDOOM_XBOX_FAST_SPANS)
    #if defined(__XBOX__)
        #define PSYDOOM_XBOX_FAST_SPANS 1
    #else
        #define PSYDOOM_XBOX_FAST_SPANS 0
    #endif
#endif
...
BEGIN_NAMESPACE(Gpu)
    ...
    uint64_t gDbgWallColTicks = 0;
    uint64_t gDbgFloorRowTicks = 0;
    uint64_t gDbgTriangleTicks = 0;
    uint64_t gDbgRectTicks = 0;
    uint32_t gDbgNumWallCols = 0;
    uint32_t gDbgNumFloorRows = 0;
    uint32_t gDbgNumTriangles = 0;
    uint32_t gDbgNumRects = 0;
    uint64_t gDbgWallPixels = 0;
    uint64_t gDbgFloorPixels = 0;
    uint64_t gDbgRectPixels = 0;
    ...
    // Timestamp counter directly: this sits in the rasteriser's hot path, so it has to be an instruction rather than
    // a call through SDL.
    struct XbDrawTimer {
        uint64_t& total;
        uint32_t& count;
        ...
        inline XbDrawTimer(uint64_t& t, uint32_t& c) noexcept : total(t), count(c), start(__rdtsc()) {}
        inline ~XbDrawTimer() noexcept { total += __rdtsc() - start; count++; }
    };

    #define XB_DRAW_TIMER(TICKS, COUNT) XbDrawTimer _xbDrawTimer(TICKS, COUNT)
#else
    #define XB_DRAW_TIMER(TICKS, COUNT) ((void) 0)
#endif
...
void initCore(Core& core, const uint16_t ramPixelW, const uint16_t ramPixelH) noexcept {
    ...
    // Rows are spaced 'ramStride' apart, which may be wider than the VRAM is.
    //
    // Padding the stride so it is not a power of two is what stops a wall column being a chain of L1 conflict misses.
    // Start at zero padding: this change is the split between width and stride, and must alter nothing observable
    // before any padding is introduced on top of it.
    const uint16_t allocW = (uint16_t) nextPow2(ramPixelW);
    const uint16_t stride = (uint16_t)(allocW + GPU_VRAM_STRIDE_PAD);

    core.pRam = new std::uint16_t[(size_t) stride * ramPixelH];
    std::memset(core.pRam, 0, sizeof(uint16_t) * (size_t) stride * ramPixelH);    // Zero init VRAM

    core.ramPixelW = allocW;
    core.ramStride = stride;
...
uint16_t vramReadU16(const Core& core, const uint16_t x, const uint16_t y) noexcept {
    ...
    return core.pRam[yt * core.ramStride + xt];
}
...
void vramWriteU16(Core& core, const uint16_t x, const uint16_t y, const uint16_t value) noexcept {
    const uint16_t xt = x & core.ramXMask;
    const uint16_t yt = y & core.ramYMask;
    core.pRam[yt * core.ramStride + xt] = value;
}
...
void updateClutCache(Core& core) noexcept {
    ...
    if (newTexFmt != TexFmt::Bpp16) {
        ...
        std::memcpy(
            &core.clutCache->bits,
            core.pRam + (core.clutY * core.ramStride + core.clutX),
...
void clearRect(Core& core, const Color16 color, const uint16_t x, const uint16_t y, const uint16_t w, const uint16_t h) noexcept {
    ...
    const uint16_t ramPixelW = core.ramPixelW;      // For clamping
    const uint16_t ramStride = core.ramStride;      // For addressing
    ...
    for (uint16_t curY = begY; curY < endY; ++curY) {
        for (uint16_t curX = begX; curX < endX; ++curX) {
            pRam[(uint32_t) curY * ramStride + curX] = color;
        }
    }
}
...
#if defined(__XBOX__)
    gDbgRectPixels += (uint64_t)((endX > begX) ? (endX - begX) : 0) * (uint64_t)((endY > begY) ? (endY - begY) : 0);
#endif
...
void draw(Core& core, const DrawRect& rect) noexcept {
    XB_DRAW_TIMER(gDbgRectTicks, gDbgNumRects);
    if (core.texFmt == TexFmt::Bpp4) {
...
static void draw(Core& core, const DrawTriangle& triangle) noexcept {
    ...
    uint16_t* pDstPixelRow = core.pRam + ty * core.ramStride;
    const bool bEnableMasking = (!core.bDisableMasking);

    for (int32_t y = ty; y <= by; ++y, pDstPixelRow += core.ramStride) {
...
void draw(Core& core, const DrawTriangle& triangle) noexcept {
    XB_DRAW_TIMER(gDbgTriangleTicks, gDbgNumTriangles);
    if (core.texFmt == TexFmt::Bpp4) {
...
void draw(Core& core, const DrawFloorRow& row) noexcept {
    XB_DRAW_TIMER(gDbgFloorRowTicks, gDbgNumFloorRows);
    sanityCheckGpuDrawState(core);
    ...
    const uint16_t vramPixelW = core.ramPixelW;     // Masking and bounds only
    const uint16_t vramStride = core.ramStride;     // Row addressing
    ...
    uint16_t* pDstPixelRow = pVram + py * vramStride;
    const bool bEnableMasking = (!core.bDisableMasking);
    ...
    for (int32_t x = lx; x <= rx; ++x) {
        ...
        if constexpr ((DrawMode == DrawMode::Textured) || (DrawMode == DrawMode::TexturedBlended)) {
            ...
            const uint16_t vramPixel = pVram[(vramY & vramYMask) * vramStride + (vramX & vramXMask)];
...
void draw(Core& core, const DrawWallCol& col) noexcept {
    XB_DRAW_TIMER(gDbgWallColTicks, gDbgNumWallCols);
    sanityCheckGpuDrawState(core);
...
#if defined(__XBOX__)
    gDbgWallPixels += (uint64_t)((by >= ty) ? (by - ty + 1) : 0);
#endif

#if PSYDOOM_XBOX_FAST_SPANS
    // Step 'v' as a fixed point number rather than interpolating it in floating point every pixel.
    //
    // The line below this does two float multiplies, an add and a cast to an integer for every single pixel of every
    // wall column. On this console's CPU the cast is the expensive part: converting a float to an integer in C means
    // truncation, and without SSE the compiler has to save the floating point control word, change the rounding mode,
    // convert, and put it back - tens of cycles, per pixel.
    //
    // None of it is necessary. 'v' is linear in 't', so it can be stepped by adding a constant, which makes the whole
    // thing one add and one shift. Measured before this: wall columns cost 8.5ms a frame, 607 of them, about 42 cycles
    // a pixel for a textured span.
    //
    // 16.16 fixed point is ample - texture coordinates here are well under 32767 - and truncating a positive number by
    // shifting gives the same answer the cast did.
    const float vSpan = v2 - v1;
    int32_t vFixed = (int32_t)((v1 + vSpan * t) * 65536.0f);
    const int32_t vStepFixed = (int32_t)(vSpan * tStep * 65536.0f);
#endif
    ...
    for (int32_t y = ty; y <= by; ++y) {
        ...
        // Ask for the texel several rows ahead before it is needed.
        //
        // This loop is not slow because of its arithmetic. Measured on hardware it costs about 310ns a pixel, which is
        // some 227 cycles - far beyond what a few masks, a lookup and a store can explain. The reason is where the
        // texels are: 'texVramX' is fixed for the whole column while 'vramY' walks down it, so every read is
        // 'vramPixelW' apart - 2048 bytes. Every texel is a different cache line, at a stride that keeps landing in
        // the same cache set, so every pixel misses both levels of cache and waits on memory.
        //
        // Nothing can make those reads contiguous without changing how textures sit in VRAM. What can be done is ask
        // for them early, so the wait happens while other pixels are being drawn rather than in front of each one.
        if constexpr ((DrawMode == DrawMode::Textured) || (DrawMode == DrawMode::TexturedBlended)) {
            const uint16_t vAhead = (uint16_t)((vFixed + vStepFixed * XB_TEX_PREFETCH_ROWS) >> 16);
            uint16_t vramYAhead = vAhead & core.texWinYMask;
            vramYAhead += core.texWinY;
            vramYAhead &= core.texPageYMask;
            vramYAhead += core.texPageY;
            vramYAhead &= core.ramYMask;

            XB_PREFETCH(&pVram[vramYAhead * vramStride + texVramX]);
        }
    #else
        const uint16_t v = (uint16_t)(v1 * tinv + v2 * t);
        ...
        if constexpr ((DrawMode == DrawMode::Textured) || (DrawMode == DrawMode::TexturedBlended)) {
            ...
            // Read the VRAM pixel and lookup the actual texel using the clut index
            const uint16_t vramPixel = pVram[vramY * vramStride + texVramX];
            const uint16_t clutIdx = (vramPixel >> ((u & 1) * 8)) & 0xFF;
            fgColor = core.clutCache[clutIdx];
        ...
        // Do blending with the background if that is enabled
        uint16_t& dstPixel = pDstPixelCol[y * vramStride];
