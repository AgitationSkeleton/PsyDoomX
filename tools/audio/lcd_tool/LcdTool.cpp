...
static bool getSoundAdpcmData(const char* const soundFilePath, std::vector<std::byte>& adpcmData) noexcept {
    ...
    if (bIsWavFile) {
        ...
        if (numChannels != 1) {
            printf("Error! Input .wav file '%s' is stereo! Only mono .wav files must be supplied!\n", errorMsg.c_str());
            return false;
        }
...
static bool dumpLcd(const char* const lcdFilePath, const char* const wmdFilePath, const char* const outputDir) noexcept {
    ...
    for (const LcdSample& sample : gLcd.samples) {
        ...
        if (!VagUtils::writePsxAdpcmSoundToVagFile(vagFilePath.c_str(), sample.adpcmData.data(), (uint32_t) sample.adpcmData.size(), sampleRate)) {
            printf("Failed to write to output .vag file '%s'! Is the path writable?\n", vagFilePath.c_str());
            return false;
        }
        ...
        if (!WavUtils::writePsxAdpcmSoundToWavFile(wavFilePath.c_str(), sample.adpcmData.data(), (uint32_t) sample.adpcmData.size(), sampleRate)) {
            printf("Failed to write output .wav file '%s'! Is the path writable?\n", wavFilePath.c_str());
            return false;
        }
    }
...
static bool listLcd(const char* const lcdFilePath, const char* const wmdFilePath) noexcept {
    ...
    printf("Patch Sample Index,    Size (In bytes),    Size (Samples),     Guessed Sample Rate,    Guessed Length (Seconds),\n");
    ...
    for (const LcdSample& sample : gLcd.samples) {
        ...
        printf("%-18u,    ", (unsigned) sample.patchSampleIdx);
        printf("%-15u,    ", (unsigned) sample.adpcmData.size());
        printf("%-14u,     ", (unsigned) numSamples);
        printf("%-19u,    ", (unsigned) sampleRate);
        printf("%-24f,\n", duration);
...
int main(int argc, const char* const argv[]) noexcept {
    ...
    if (std::strcmp(cmdSwitch, "-list") == 0) {
        ...
        try {
            ...
            printf("One or more patch sample indexes are malformed! Patch sample indexes must be integers between 0-65535.\n\n");
            printHelp();
            return 1;
        }
